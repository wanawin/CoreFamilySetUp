#!/usr/bin/env python3
"""
Core Affinity Lab v21 — Profile Decomposition Engine
...
APP_VERSION = "v21"
BUILD_MARKER = "BUILD: core_affinity_lab_v21_PROFILE_DECOMPOSITION_ENGINE__2026-06-17"
DEPLOY_FILENAME_NOTE = "For Streamlit Cloud deployment, this file may be renamed to: core_affinity_lab_v1 (1).py"

st.set_page_config(page_title="Core Affinity Lab v21", layout="wide")
st.title("Core Affinity Lab v21 — Profile Decomposition Engine")
...
def build_profiles(rows: pd.DataFrame, level: str, cfg: ProfileCfg, target_limit: int | None = None) -> Tuple[pd.DataFrame, pd.DataFrame]:
    if rows.empty:
    ...
    profile_rows = []
    residual_rows = []
    trait_list = rows["trait_list"].tolist()
    for target in targets:
        ...
        for pnum in range(1, cfg.max_profiles_per_target + 1):
            ...
            best_score = -1.0
            for t in pool:
                stats = _score_stack(rows, level, target, (t,), all_idxs)
                if stats["target_hits"] < cfg.min_target_hits or stats["precision"] < cfg.min_precision:
                    continue
                # coverage-first: target hits dominate; precision and margin break ties
                score = stats["target_hits"] * (0.75 + stats["precision"]) + max(0, stats["target_hits"] - stats["sibling_hits"]) * 0.15
                if score > best_score:
                    best_score = score; best_stack=(t,); best_stats=stats
            if best_stack is None:
                break
            # Greedily refine stack up to depth, but only if target hits do not collapse.
            current = best_stack
            current_stats = best_stats
            for depth in range(2, cfg.max_stack_depth + 1):
                add_best = None; add_stats = None; add_score = -1.0
                # candidate add-ons from currently covered target rows
                matched = _rows_matching_stack(rows, current, remaining)
                addon_counts = _trait_counts(rows, matched)
                for t, _n in addon_counts.most_common(cfg.top_trait_pool):
                    if t in current: continue
                    cand = tuple(sorted(current + (t,)))
                    stats = _score_stack(rows, level, target, cand, all_idxs)
                    if stats["target_hits"] < max(cfg.min_target_hits, math.ceil(current_stats["target_hits"] * 0.45)):
                        continue
                    if stats["precision"] < current_stats["precision"]:
                        continue
                    score = stats["target_hits"] * (0.65 + stats["precision"]) + max(0, stats["target_hits"] - stats["sibling_hits"]) * 0.25
                    if score > add_score:
            ...
            profile_rows.append({
                ...
                "target_hits": int(current_stats["target_hits"]),
...
with st.sidebar:
    ...
    st.caption("All 120 cores / 360 members are included by default. Any limits are only performance safety limits.")
...
try:
    ...
    texts = {"README_v21.txt": f"{BUILD_MARKER}\n{DEPLOY_FILENAME_NOTE}\nStage run: {stage}\nDiscovery lab only.\n"}
    ...
    if stage.startswith("1"):
        ...
        with st.spinner("Building core coverage-first profiles..."):
            prof, residual = build_profiles(rows, "core", cfg_core)
        frames.update({"core_decomposed_profiles.csv": prof.head(export_top_n), "core_profile_residuals.csv": residual})
        ...
        with st.spinner("Building member coverage-first profiles..."):
            prof, residual = build_profiles(rows, "member", cfg_member)
        frames.update({"member_decomposed_profiles.csv": prof.head(export_top_n), "member_profile_residuals.csv": residual})
    ...
    st.download_button(
        "Download selected v21 stage outputs ZIP",
        package,
        file_name="core_affinity_lab_v21_selected_stage_outputs.zip",
        mime="application/zip",
    )

except Exception as e:
    st.error("v21 stage failed. Full traceback below.")
    st.exception(e)
