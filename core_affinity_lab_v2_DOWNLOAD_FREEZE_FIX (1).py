...
BUILD_MARKER = "BUILD: core_affinity_lab_v2__2026-06-16_DOWNLOAD_FREEZE_FIX"
...
st.set_page_config(page_title="Core Affinity Lab v2", layout="wide")
st.title("Core Affinity Lab v2")
st.caption(BUILD_MARKER)
st.info(
    "Research-only lab: mines seed-trait, stream, member, and cadence affinity for all 120 AABC core families. "
    "No playlist, no cuts, no RTE, no B1Z0, no rescues, and no betting logic. v2 freezes export bytes so downloads do not reset completed results."
)
...
def make_zip(named_dfs: Dict[str, pd.DataFrame], readme: str) -> bytes:
    ...
    with zipfile.ZipFile(bio, "w", compression=zipfile.ZIP_DEFLATED, allowZip64=True) as zf:
        zf.writestr("README_core_affinity_lab_v2.txt", readme.encode("utf-8"))
        for name, df in named_dfs.items():
            zf.writestr(name, bytes_csv(df))
    return bio.getvalue()


def freeze_affinity_outputs(outputs: Dict[str, pd.DataFrame], readme: str) -> None:
    """Freeze completed lab outputs so download clicks do not require rerunning the app.

    Streamlit reruns scripts on button clicks. The lab run button is momentary, so a
    download click can otherwise rerun with run_button=False and appear to reset.
    This stores CSV/ZIP bytes in session_state after a successful run. It does not
    change any mining, scoring, ranking, or accuracy logic.
    """
    frozen_csv = {name: bytes_csv(df) for name, df in (outputs or {}).items()}
    st.session_state["affinity_lab_v2_outputs"] = outputs
    st.session_state["affinity_lab_v2_csv_bytes"] = frozen_csv
    st.session_state["affinity_lab_v2_zip_bytes"] = make_zip(outputs, readme)
    st.session_state["affinity_lab_v2_exports_ready"] = True


def get_download_kwargs():
    """Avoid download-click reruns on Streamlit versions that support it."""
    return {"on_click": "ignore"}


def render_affinity_downloads(outputs: Dict[str, pd.DataFrame], readme: str) -> None:
    """Render downloads from frozen bytes when available."""
    if not st.session_state.get("affinity_lab_v2_exports_ready"):
        freeze_affinity_outputs(outputs, readme)
    zip_bytes = st.session_state.get("affinity_lab_v2_zip_bytes", b"")
    csv_bytes = st.session_state.get("affinity_lab_v2_csv_bytes", {})
    st.download_button(
        "Download ALL Core Affinity Lab outputs ZIP",
        data=zip_bytes,
        file_name="core_affinity_lab_v2_outputs.zip",
        mime="application/zip",
        use_container_width=True,
        key="download_affinity_lab_v2_all_zip",
        **get_download_kwargs(),
    )
    for name in outputs.keys():
        st.download_button(
            f"Download {name}",
            data=csv_bytes.get(name, b""),
            file_name=name,
            mime="text/csv",
            use_container_width=True,
            key=f"download_affinity_lab_v2_{name}",
            **get_download_kwargs(),
        )
...
if not run_button and not st.session_state.get("affinity_lab_v2_exports_ready"):
    st.info("Choose settings, then click Run Core Affinity Lab.")
    st.stop()

if not run_button and st.session_state.get("affinity_lab_v2_exports_ready"):
    st.success("Previous Core Affinity Lab run is still loaded. Download buttons below use frozen export bytes and will not rebuild/reset results.")
    cached_outputs = st.session_state.get("affinity_lab_v2_outputs", {})
    if isinstance(cached_outputs, dict) and cached_outputs:
        st.subheader("Downloads")
        render_affinity_downloads(cached_outputs, st.session_state.get("affinity_lab_v2_readme", "Core Affinity Lab v2 frozen exports."))
        st.stop()
    st.info("Choose settings, then click Run Core Affinity Lab.")
    st.stop()
...
"""
st.session_state["affinity_lab_v2_readme"] = readme
freeze_affinity_outputs(outputs, readme)
render_affinity_downloads(outputs, readme)
