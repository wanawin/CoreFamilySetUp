#!/usr/bin/env python3
"""
Core Affinity Lab v31 — Global Core Separator Miner
...
APP_VERSION = "v31"
BUILD_MARKER = "BUILD: core_affinity_lab_v31_GLOBAL_CORE_SEPARATOR_MINER__2026-06-18"
DEPLOY_FILENAME_NOTE = "For Streamlit Cloud deployment, this file may be renamed to: core_affinity_lab_v1 (1).py"

st.set_page_config(page_title="Core Affinity Lab v31", layout="wide")
st.title("Core Affinity Lab v31 — Global Core Separator Miner")
st.caption(BUILD_MARKER)
st.info(
    "Upload clean history plus all v30 Stage-2 core-profile ZIP chunks. "
    "v31 merges v30 coverage buckets globally, deduplicates overlaps, then mines separators inside each broad bucket. "
...
def mine_profile_separators(
    ...
    for n, (_, pr) in enumerate(selected.iterrows(), start=1):
        ...
        if status is not None:
            status.info(f"Mining separators: profile {n}/{total} — core {target} profile {pr.get('profile_num','')}")
        ...
        audit_rows.append({
            ...
            "base_precision": round(base_precision, 5),
            "base_target_coverage": round(base_target_coverage, 5),
        ...
        for trait, _cnt in candidate_counts.most_common(candidate_pool):
            if trait in base_stack:
                continue
            matched = bucket & trait_index.get(trait, set())
            target_hits = len(matched & target_idxs)
            if target_hits < min_target_hits:
                continue
            sample = len(matched)
            sibling_hits = sample - target_hits
            precision = target_hits / sample if sample else 0.0
            profile_coverage = target_hits / len(bucket_target) if bucket_target else 0.0
            global_coverage = target_hits / total_target_hits if total_target_hits else 0.0
            if precision < min_precision and profile_coverage < min_profile_coverage:
                continue
            margin = target_hits - sibling_hits
            score = (precision * 100.0) + (profile_coverage * 60.0) + (target_hits * 1.5) + max(0, margin) * 0.15
            row = {
                "profile_row_num": int(pr.get("profile_row_num", n)),
                "target": target,
                "profile_id": pr.get("profile_id", ""),
                ...
                "profile_coverage": round(profile_coverage, 5),
                "global_target_coverage": round(global_coverage, 5),
                "hit_margin": margin,
                "score": round(score, 5),
            ...
            sep_rows.append(row)
            primary_candidates.append((score, trait, matched, row))

        if refine_depth >= 2 and primary_candidates:
            # bounded second separator trait search inside strongest primary candidates
            primary_candidates = sorted(primary_candidates, key=lambda x: x[0], reverse=True)[:15]
            for _score, trait1, match1, _row1 in primary_candidates:
                target_after_1 = match1 & target_idxs
                if len(target_after_1) < min_target_hits:
                    continue
                counts2 = count_traits(rows, target_after_1)
                for trait2, _c2 in counts2.most_common(min(candidate_pool, 60)):
                    if trait2 in base_stack or trait2 == trait1:
                        continue
                    matched2 = match1 & trait_index.get(trait2, set())
                    target_hits = len(matched2 & target_idxs)
                    if target_hits < min_target_hits:
                        continue
                    sample = len(matched2)
                    sibling_hits = sample - target_hits
                    ...
                    if precision < min_precision and profile_coverage < min_profile_coverage:
                        continue
                    margin = target_hits - sibling_hits
                    score = (precision * 130.0) + (profile_coverage * 50.0) + (target_hits * 1.2) + max(0, margin) * 0.2
                    sep_rows.append({
                        "profile_row_num": int(pr.get("profile_row_num", n)),
                        "target": target,
                        "profile_id": pr.get("profile_id", ""),
    ...
    if status is not None:
        status.success(f"Finished separator mining for {len(selected)} profiles in {time.time()-t0:.2f} seconds.")
    seps = pd.DataFrame(sep_rows)
    if not seps.empty:
        seps = seps.sort_values(["score", "precision", "profile_coverage", "target_hits"], ascending=[False, False, False, False]).reset_index(drop=True)
...
with st.sidebar:
    ...
    st.caption("v31 is manifest-only by default. It mines separator candidates inside v30 coverage buckets; outputs are discovery-only.")
...
if "v31_package" not in st.session_state:
    st.session_state["v31_package"] = None
    st.session_state["v31_manifest"] = None
    st.session_state["v31_filename"] = None

run = st.button("Run selected v31 stage and prepare frozen ZIP", type="primary")
...
if run:
    ...
    try:
        ...
        if stage.startswith("1"):
            ...
            frames["v31_zip_manifest.csv"] = zip_manifest
        ...
        manifest = pd.DataFrame([{"metric": k, "value": v} for k, v in metrics.items()])
        frames["summary.csv"] = manifest
        texts["README_v31.txt"] = (
            ...
            "v31 consumes v30 coverage profiles and mines separator traits inside profile collision buckets.\n"
        )
        package = zip_frames(frames, texts)
        filename = f"core_affinity_lab_v31_stage_{stage.split()[0]}_profiles_{int(profile_start)}.zip"
        st.session_state["v31_package"] = package
        st.session_state["v31_manifest"] = manifest
        st.session_state["v31_filename"] = filename
        # drop big frames before rendering UI
        del frames, profiles, residuals, zip_manifest, package
        gc.collect()
        st.success("v31 frozen ZIP prepared. Download below. No rerun/rebuild is needed.")

    except Exception as e:
        st.error("v31 stage failed. Full traceback below.")
        st.exception(e)

if st.session_state.get("v31_manifest") is not None:
    st.subheader("Frozen package manifest")
    st.table(st.session_state["v31_manifest"])

if st.session_state.get("v31_package") is not None:
    st.download_button(
        "Download frozen v31 stage outputs ZIP",
        st.session_state["v31_package"],
        file_name=st.session_state.get("v31_filename", "core_affinity_lab_v31_outputs.zip"),
        mime="application/zip",
        key="v31_download_button",
    )
