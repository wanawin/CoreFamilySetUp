...
APP_VERSION = "v26"
BUILD_MARKER = "BUILD: core_affinity_lab_v26_SAFE_DOWNLOAD_NO_RERUN__2026-06-18"
DEPLOY_FILENAME_NOTE = "For Streamlit Cloud deployment, this file may be renamed to: core_affinity_lab_v1 (1).py"

st.set_page_config(page_title="Core Affinity Lab v26", layout="wide")
st.title("Core Affinity Lab v26 — Safe Download / No Rerun Fix")
st.caption(BUILD_MARKER)
st.info(
    "Coverage-first lab: build multiple profiles per core/member, then mine separators only for collisions. v26 uses safe staged execution: run once, freeze ZIP, then download without rebuilding. "
...
with st.sidebar:
    ...
    export_top_n = st.slider("CSV export top N rows", 25, 2000, 250, 25)
    st.divider()
    ...
    st.caption("v26 does not auto-run after upload. Click Run once to prepare a frozen ZIP, then download it without rebuilding.")

if not upload:
    st.stop()

# Stable key identifies the intended job. If settings change, old ZIP is hidden.
...
try:
    ...
    if "v26_package" not in st.session_state:
        st.session_state["v26_package"] = None
        st.session_state["v26_job_key"] = None
        st.session_state["v26_file_name"] = None
        st.session_state["v26_manifest"] = None

    run_now = st.button("Run selected v26 stage and prepare frozen ZIP", type="primary")
    ...
    if run_now:
        ...
        summary = pd.DataFrame([
            ...
            {"metric":"download_mode", "value":"frozen_session_state_no_rebuild_on_download"},
            {"metric":"note", "value":"Profile decomposition is discovery-only; not production play logic."},
        ])
        frames = {"summary.csv": summary}
        texts = {"README_v26.txt": f"{BUILD_MARKER}\n{DEPLOY_FILENAME_NOTE}\nStage run: {stage}\nDiscovery lab only. v26 freezes ZIP output in session_state and avoids preview rendering/rebuild on download.\n"}
        manifest_rows = []

        if stage.startswith("1"):
            with st.spinner("Building base summaries..."):
                ...
                manifest_rows.append({"file":"core_base_counts.csv", "rows":len(core_counts)})
                manifest_rows.append({"file":"member_base_counts.csv", "rows":len(member_counts)})
                manifest_rows.append({"file":"stream_core_base_counts.csv", "rows":min(len(stream_core), export_top_n)})
            ...
            debug = debug_candidate_profiles(rows, "core", first_target, cfg_core, max_rows=min(250, export_top_n))
            frames.update({
                "core_decomposed_profiles.csv": prof.head(export_top_n),
                "core_profile_residuals.csv": residual,
                "core_prefilter_candidate_debug.csv": debug,
            })
            manifest_rows += [
                {"file":"core_decomposed_profiles.csv", "rows":len(prof.head(export_top_n)), "full_rows_found":len(prof)},
                {"file":"core_profile_residuals.csv", "rows":len(residual)},
                {"file":"core_prefilter_candidate_debug.csv", "rows":len(debug)},
            ]
            st.success(f"Stage 2 complete. Profiles found: {len(prof):,}. Residual targets: {len(residual):,}.")
            ...
            debug = debug_candidate_profiles(rows, "member", first_target, cfg_member, max_rows=min(250, export_top_n))
            frames.update({
                "member_decomposed_profiles.csv": prof.head(export_top_n),
                "member_profile_residuals.csv": residual.head(export_top_n),
                "member_prefilter_candidate_debug.csv": debug,
            })
            manifest_rows += [
                {"file":"member_decomposed_profiles.csv", "rows":len(prof.head(export_top_n)), "full_rows_found":len(prof)},
                {"file":"member_profile_residuals.csv", "rows":len(residual.head(export_top_n)), "full_rows_found":len(residual)},
                {"file":"member_prefilter_candidate_debug.csv", "rows":len(debug)},
            ]
            st.success(f"Stage 3 complete. Profiles found: {len(prof):,}. Residual targets: {len(residual):,}.")
            ...
            manifest_rows += [
                {"file":"core_decomposed_profiles.csv", "rows":len(prof.head(export_top_n)), "full_rows_found":len(prof)},
                {"file":"core_profile_collisions.csv", "rows":len(collisions.head(export_top_n)), "full_rows_found":len(collisions)},
                {"file":"core_collision_separator_refinements.csv", "rows":len(sep.head(export_top_n)), "full_rows_found":len(sep)},
            ]
            st.success(f"Stage 4 complete. Profiles: {len(prof):,}; collisions: {len(collisions):,}; refinements: {len(sep):,}.")
            ...
            manifest_rows += [
                {"file":"member_decomposed_profiles.csv", "rows":len(prof.head(export_top_n)), "full_rows_found":len(prof)},
                {"file":"member_profile_collisions.csv", "rows":len(collisions.head(export_top_n)), "full_rows_found":len(collisions)},
                {"file":"member_collision_separator_refinements.csv", "rows":len(sep.head(export_top_n)), "full_rows_found":len(sep)},
            ]
            st.success(f"Stage 5 complete. Profiles: {len(prof):,}; collisions: {len(collisions):,}; refinements: {len(sep):,}.")

        manifest = pd.DataFrame(manifest_rows) if manifest_rows else pd.DataFrame([{"file":"summary.csv", "rows":len(summary)}])
        frames["manifest.csv"] = manifest
        package = zip_frames(frames, texts)
        out_name = f"core_affinity_lab_v26_stage_{stage.split()[0]}_chunk_{int(target_start)}.zip"
        st.session_state["v26_package"] = package
        st.session_state["v26_job_key"] = job_key
        st.session_state["v26_file_name"] = out_name
        st.session_state["v26_manifest"] = manifest
        st.success(f"Frozen ZIP prepared: {out_name} ({len(package)/1024:,.1f} KB).")

    st.subheader("Frozen download")
    if st.session_state.get("v26_package") is not None and st.session_state.get("v26_job_key") == job_key:
        st.download_button(
            "Download frozen v26 ZIP — no rebuild",
            st.session_state["v26_package"],
            file_name=st.session_state["v26_file_name"],
            mime="application/zip",
            key="download_frozen_v26_zip",
        )
        if st.session_state.get("v26_manifest") is not None:
            st.caption("ZIP manifest")
            st.table(st.session_state["v26_manifest"])
    else:
        st.info("No frozen ZIP for the current settings yet. Click the Run button once. Changing stage/settings requires a new run.")

except Exception as e:
    st.error("v26 stage failed. Full traceback below.")
    st.exception(e)
