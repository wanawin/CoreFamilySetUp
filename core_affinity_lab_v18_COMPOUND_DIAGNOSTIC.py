#!/usr/bin/env python3
"""
Core Affinity Lab v18 — COMPOUND SEPARATOR DIAGNOSTIC
...
APP_VERSION = "v18"
BUILD_MARKER = "BUILD: core_affinity_lab_v18_COMPOUND_DIAGNOSTIC__2026-06-17"
DIGITS = "0123456789"
...
st.set_page_config(page_title="Core Affinity Lab v18", layout="wide")
st.title("Core Affinity Lab v18 — Compound Separator Diagnostics")
st.caption(BUILD_MARKER)
st.info("v18 adds compound mining diagnostics: stages 10–13 now export candidate counts and debug rows before/after filters so empty separator outputs can be diagnosed. Lab only; ALL 120 cores / ALL 360 members by default.")
        ...
        for pair in pairs:
            ...
            for favored, hits in [(a, ha), (b, hb)]:
                ...
                if row:
                    ...
                    row.update({"separator_type": separator_type, "pair": f"{a}_vs_{b}", "combo_size": combo_size})
                    if core_for_member_pair:
...
with st.sidebar:
    ...
    compound_base_limit = st.slider("Compound miner: base single-trait pool", 20, 300, 80, 10)
    compound_row_trait_cap = st.slider("Compound miner: max traits per row", 8, 40, 18, 2)
    compound_combo_cap = st.slider("Compound miner: max combos per row", 50, 1000, 250, 50)

uploaded = st.file_uploader("Upload full clean Pick-4 history", type=["csv", "txt", "tsv", "xlsx", "xls"])
if not uploaded:
    st.stop()

if st.button("Run selected v18 stage", type="primary", use_container_width=True):
    ...
    try:
        ...
        if stage_num == 1:
            ...
            df, base_df, debug_df, diag_df = validate_all_core_pair_compound_separators(trans, target_cores, min_sample, min_hits, min_lift, stable_den, top_n, compound_base_limit, 2, compound_row_trait_cap, compound_combo_cap)
            ...
            df, base_df, debug_df, diag_df = validate_within_core_member_compound_separators(trans, target_cores, min_sample, min_hits, min_lift, stable_den, top_n, compound_base_limit, 2, compound_row_trait_cap, compound_combo_cap)
            ...
            df, base_df, debug_df, diag_df = validate_all_core_pair_compound_separators(trans, target_cores, min_sample, min_hits, min_lift, stable_den, top_n, compound_base_limit, 3, compound_row_trait_cap, compound_combo_cap)
            ...
            df, base_df, debug_df, diag_df = validate_within_core_member_compound_separators(trans, target_cores, min_sample, min_hits, min_lift, stable_den, top_n, compound_base_limit, 3, compound_row_trait_cap, compound_combo_cap)
        ...
        zip_bytes = make_zip(frames, {"README_v18.txt": "Core Affinity Lab v18 compound-separator diagnostic output. Includes stage execution metadata, filter pass counts, and debug candidate rows for compound separator stages. Lab only; no playlist logic."})
        freeze_bytes("v17_last_zip", zip_bytes)
        st.success("Stage complete. Download ZIP below.")
        st.download_button("Download v17 selected-stage outputs ZIP", data=zip_bytes, file_name="core_affinity_lab_v17_selected_stage_outputs.zip", mime="application/zip", use_container_width=True)

    except Exception as e:
        st.error("v17 caught the error instead of blank-crashing.")
        st.exception(e)
        st.stop()

if "v17_last_zip" in st.session_state:
    st.download_button("Download previous v17 ZIP again", data=st.session_state["v17_last_zip"], file_name="core_affinity_lab_v17_selected_stage_outputs.zip", mime="application/zip", use_container_width=True)
