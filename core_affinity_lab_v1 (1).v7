#!/usr/bin/env python3
"""
Core Affinity Lab v7 — Universal Trait + Member Profiles
...
APP_VERSION = "v7"
BUILD_MARKER = "BUILD: core_affinity_lab_v7_UNIVERSAL_TRAIT_MEMBER_PROFILES__2026-06-17"

st.set_page_config(page_title="Core Affinity Lab v7", layout="wide")
st.title("Core Affinity Lab v7 — Universal Trait + Member Profiles")
st.caption(BUILD_MARKER)
st.info(
    "Lab only. Mines universal seed traits across all 120 AABC cores and all 360 AABC members. "
    "No daily playlist, no cuts, no RTE, no B1Z0, no ZLT, no rescue logic."
)
...
def _freeze_outputs(outputs: Dict[str, pd.DataFrame], texts: Dict[str, str] | None = None) -> bytes:
    ...
    data = bio.getvalue()
    st.session_state["v7_outputs"] = outputs
    st.session_state["v7_texts"] = texts
    st.session_state["v7_zip_bytes"] = data
    return data


def _download_df(label: str, key: str, filename: str):
    outputs = st.session_state.get("v7_outputs", {})
    df = outputs.get(key)
    if isinstance(df, pd.DataFrame):
        st.download_button(
            label,
...
def build_single_trait_lifts(aabc: pd.DataFrame, min_stable_sample: int) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    total = len(aabc)
    exploded = aabc[["TransitionID", "StreamKey", "ActualCore", "ActualMember", "TraitList"]].explode("TraitList").rename(columns={"TraitList": "trait"})
    exploded = exploded[exploded["trait"].notna()].copy()
    exploded["trait_category"] = exploded["trait"].map(trait_category)
    core_lift = lift_by_group(exploded, ["trait", "trait_category"], "ActualCore", base_core_counts, total, min_stable_sample)
    member_lift = lift_by_group(exploded, ["trait", "trait_category"], "ActualMember", base_member_counts, total, min_stable_sample)
    trait_dict = exploded.groupby(["trait", "trait_category"]).size().rename("transition_count").reset_index().sort_values("transition_count", ascending=False)
    return trait_dict, core_lift, member_lift
...
def build_stream_trait_lifts(aabc: pd.DataFrame, min_stable_sample: int) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    exploded = exploded[exploded["trait"].notna()].copy()
    exploded["trait_category"] = exploded["trait"].map(trait_category)
    core = lift_by_group(exploded, ["StreamKey", "trait", "trait_category"], "ActualCore", base_core_counts, total, min_stable_sample)
    member = lift_by_group(exploded, ["StreamKey", "trait", "trait_category"], "ActualMember", base_member_counts, total, min_stable_sample)
    return core, member


def make_stacks_for_rows(aabc: pd.DataFrame, top_traits: set[str], max_traits_per_row: int, stack_size: int,
                         max_rows: int | None = None) -> pd.DataFrame:
    rows = []
    source = aabc if max_rows is None else aabc.head(max_rows)
    for _, r in source.iterrows():
        traits = [t for t in r["TraitList"] if t in top_traits]
        # Keep per-row combinations bounded and deterministic.
        traits = traits[:max_traits_per_row]
        for combo in itertools.combinations(traits, stack_size):
            rows.append({
                "TransitionID": r["TransitionID"],
                "StreamKey": r["StreamKey"],
                "ActualCore": r["ActualCore"],
                "ActualMember": r["ActualMember"],
                "stack_trait": " && ".join(combo),
...
def build_stacked_lifts(aabc: pd.DataFrame, trait_core_lift: pd.DataFrame, min_stable_sample: int,
    ...
    # Use high-confidence single traits as base atoms to prevent combinatorial explosion.
    top_traits = set(trait_core_lift.head(max_base_traits)["trait"].astype(str).tolist())
    frames = []
    if include_pairs:
        frames.append(make_stacks_for_rows(aabc, top_traits, max_traits_per_row, 2))
    if include_triples:
        frames.append(make_stacks_for_rows(aabc, top_traits, max_traits_per_row, 3))
    if not frames:
        return pd.DataFrame(), pd.DataFrame()
    stacked = pd.concat([f for f in frames if not f.empty], ignore_index=True) if any(not f.empty for f in frames) else pd.DataFrame()
    if stacked.empty:
        return pd.DataFrame(), pd.DataFrame()
    base_core_counts = aabc["ActualCore"].value_counts()
    base_member_counts = aabc["ActualMember"].value_counts()
    total = len(aabc)
    core = lift_by_group(stacked, ["stack_trait", "stack_size"], "ActualCore", base_core_counts, total, min_stable_sample)
    member = lift_by_group(stacked, ["stack_trait", "stack_size"], "ActualMember", base_member_counts, total, min_stable_sample)
    return core, member
...
def build_profiles(aabc: pd.DataFrame, min_stable_sample: int) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    member_profiles["member_role_by_core_freq"] = member_profiles.groupby("core_id")["hits"].rank(method="first", ascending=False).map({1.0:"strongest_candidate",2.0:"middle_candidate",3.0:"suppressed_candidate"}).fillna("unknown")
    ...
    stream_core = stream_core.merge(core_profiles[["core_id", "baseline_rate"]], left_on="ActualCore", right_on="core_id", how="left").drop(columns=["core_id"])
    ...
    stream_member = stream_member.merge(member_profiles[["member", "baseline_rate"]], left_on="ActualMember", right_on="member", how="left").drop(columns=["member"])
...
export_top_n = st.sidebar.slider("Export top N per heavy table", 100, 10000, 3000, 100)
include_pair_stacks = st.sidebar.checkbox("Mine 2-trait stacked signals", value=True)
include_triple_stacks = st.sidebar.checkbox("Mine 3-trait stacked signals", value=False)
max_base_traits = st.sidebar.slider("Stack miner: max base single traits", 25, 500, 150, 25)
max_traits_per_row = st.sidebar.slider("Stack miner: max traits per row", 5, 30, 12, 1)
...
run_btn = st.button("Run v7 universal trait/member profiler", type="primary", use_container_width=True)
...
if run_btn:
    ...
    zip_bytes = _freeze_outputs(outputs, {"README_v7.txt": readme})
    progress.progress(100, text="v7 complete.")

if "v7_outputs" not in st.session_state:
    st.stop()

outputs = st.session_state["v7_outputs"]
st.success("v7 outputs ready. Download buttons use frozen session bytes.")
...
st.download_button(
    "Download all v7 outputs ZIP",
    data=st.session_state.get("v7_zip_bytes", b""),
    file_name="core_affinity_lab_v7_all_outputs.zip",
    mime="application/zip",
    key="download_v7_all_zip",
    use_container_width=True,
)
for key in [
