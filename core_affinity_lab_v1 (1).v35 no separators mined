#!/usr/bin/env python3
"""
Core Affinity Lab v35 — Instrumented Capped Beam Miner
...
APP_VERSION = "v35"
BUILD_MARKER = "BUILD: core_affinity_lab_v35_INSTRUMENTED_CAPPED_BEAM_MINER__2026-06-18"
DEPLOY_FILENAME_NOTE = "For Streamlit Cloud deployment, this file may be renamed to: core_affinity_lab_v1 (1).py"

st.set_page_config(page_title="Core Affinity Lab v35", layout="wide")
st.title("Core Affinity Lab v35 — Instrumented Capped Beam Miner")
st.caption(BUILD_MARKER)
st.info(
    "Upload clean history plus all v30 Stage-2 core-profile ZIP chunks. "
    "v35 merges v30 coverage buckets globally, deduplicates overlaps, then mines INSTRUMENTED CAPPED beam-search separators: target rows vs sibling rows inside each broad bucket. "
...
def mine_profile_separators(
    ...
    """v35 instrumented/capped beam-search collision separator miner.

    v34 proved the beam idea but could kill Streamlit on profile 1. v35 is intentionally
    ...
    for n, (_, pr) in enumerate(selected.iterrows(), start=1):
        ...
        if status is not None:
            status.info(f"v35 capped beam: profile {n}/{total} — core {target} profile {pr.get('profile_num','')} — building bucket")
        ...
        audit = {
            ...
            "stopped_reason": "",
            "note": "v35 capped/instrumented beam search; profile chunks should start small and scale only after download success.",
        }
        ...
        if status is not None:
            status.info(f"v35 capped beam: profile {n}/{total} — target rows {len(bucket_target):,}, siblings {len(bucket_sibling):,}, level1 traits {len(level1_traits):,}")
        ...
        for trait in level1_traits:
            ...
            if not stats or stats["target_hits"] < min_target_hits:
                continue
            if stats["target_vs_sibling_lift"] <= 1.01 and stats["precision_gain"] <= 0:
                continue
            sep_stack = (trait,)
            seen.add(sep_stack)
        ...
        for depth in range(2, max_depth + 1):
            ...
            for _score, sep_stack, matched_prev, stats_prev in current_beam:
                ...
                for trait, _cnt in addon_counts.most_common(addon_pool):
                    ...
                    if not stats or stats["target_hits"] < min_target_hits:
                        continue
                    if stats["precision"] + 1e-9 < stats_prev["precision"] and stats["target_vs_sibling_lift"] <= stats_prev["target_vs_sibling_lift"]:
                        continue
                    if (stats["precision_gain"] <= 0 and stats["target_vs_sibling_lift"] <= 1.05 and stats["hit_margin"] <= stats_prev["hit_margin"]):
                        continue
    ...
    if status is not None:
        status.success(f"Finished v35 capped beam mining for {len(selected)} profiles in {time.time()-t0:.2f} seconds.")
...
with st.sidebar:
    ...
    st.caption("v34 is manifest-only by default. It uses bounded beam-search separator stacking against target-vs-sibling rows; outputs are discovery-only.")
...
if "v35_package" not in st.session_state:
    st.session_state["v35_package"] = None
    st.session_state["v35_manifest"] = None
    st.session_state["v35_filename"] = None

run = st.button("Run selected v34 stage and prepare frozen ZIP", type="primary")
...
if run:
    ...
    try:
        ...
        if stage.startswith("1"):
            ...
            frames["v34_zip_manifest.csv"] = zip_manifest
        ...
        texts["README_v35.txt"] = (
            ...
            "v35 consumes v30 coverage profiles and mines capped/instrumented discriminative beam-search separator stacks using target-vs-sibling frequency, lift, odds ratio, and precision gain. Hard caps prevent profile-1 candidate explosion.\n"
        )
        package = zip_frames(frames, texts)
        filename = f"core_affinity_lab_v35_stage_{stage.split()[0]}_profiles_{int(profile_start)}.zip"
        st.session_state["v35_package"] = package
        st.session_state["v35_manifest"] = manifest
        st.session_state["v35_filename"] = filename
        # drop big frames before rendering UI
        del frames, profiles, residuals, zip_manifest, package
        gc.collect()
        st.success("v34 frozen ZIP prepared. Download below. No rerun/rebuild is needed.")

    except Exception as e:
        st.error("v34 stage failed. Full traceback below.")
        st.exception(e)

if st.session_state.get("v35_manifest") is not None:
    st.subheader("Frozen package manifest")
    st.table(safe_display_df(st.session_state["v35_manifest"]))

if st.session_state.get("v35_package") is not None:
    st.download_button(
        "Download frozen v34 stage outputs ZIP",
        st.session_state["v35_package"],
        file_name=st.session_state.get("v35_filename", "core_affinity_lab_v34_outputs.zip"),
        mime="application/zip",
        key="v34_download_button",
    )
