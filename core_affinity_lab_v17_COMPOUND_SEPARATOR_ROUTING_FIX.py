#!/usr/bin/env python3
"""
Core Affinity Lab v17 — COMPOUND SEPARATOR ROUTING FIX
...
APP_VERSION = "v17"
BUILD_MARKER = "BUILD: core_affinity_lab_v17_COMPOUND_SEPARATOR_ROUTING_FIX__2026-06-17"
DIGITS = "0123456789"
...
st.set_page_config(page_title="Core Affinity Lab v17", layout="wide")
st.title("Core Affinity Lab v17 — Compound Separator Miner ROUTING FIX")
st.caption(BUILD_MARKER)
st.info("v17 fixes stage routing: stages 10–13 now execute compound separator mining instead of being misrouted to Stage 1. Keeps v15/v16 universal trait universe for ALL 120 cores and ALL 360 members.")
...
if st.button("Run selected v17 stage", type="primary", use_container_width=True):
    ...
    try:
        ...
        if stage_num == 1:
            ...
            df, base_df = validate_all_core_pair_compound_separators(trans, target_cores, min_sample, min_hits, min_lift, stable_den, top_n, compound_base_limit, 2, compound_row_trait_cap, compound_combo_cap)
            ...
            df, base_df = validate_within_core_member_compound_separators(trans, target_cores, min_sample, min_hits, min_lift, stable_den, top_n, compound_base_limit, 2, compound_row_trait_cap, compound_combo_cap)
            ...
            df, base_df = validate_all_core_pair_compound_separators(trans, target_cores, min_sample, min_hits, min_lift, stable_den, top_n, compound_base_limit, 3, compound_row_trait_cap, compound_combo_cap)
            ...
            df, base_df = validate_within_core_member_compound_separators(trans, target_cores, min_sample, min_hits, min_lift, stable_den, top_n, compound_base_limit, 3, compound_row_trait_cap, compound_combo_cap)
            frames["member_vs_member_stack3_separators.csv"] = df
            frames["compound_base_traits_member_stack3.csv"] = base_df
            st.dataframe(df, use_container_width=True, hide_index=True)

        zip_bytes = make_zip(frames, {"README_v17.txt": "Core Affinity Lab v17 compound-separator output. Routing fixed by exact numeric stage parsing. Run one stage at a time; combine exported CSVs later. Lab only; no playlist logic."})
