#!/usr/bin/env python3
# Core Affinity Lab v3 — Profile Builder
# BUILD: core_affinity_lab_v3__2026-06-16_PROFILE_BUILDER_STREAM_CORE_SEED_CADENCE
...
BUILD_MARKER = "BUILD: core_affinity_lab_v3__2026-06-16_PROFILE_BUILDER_STREAM_CORE_SEED_CADENCE"
ALL_DIGITS = "0123456789"

st.set_page_config(page_title="Core Affinity Lab v3", layout="wide")
st.title("Core Affinity Lab v3 — Profile Builder")
st.caption(BUILD_MARKER)
st.info(
    "Lab-only profile builder. Mines stream, core, stream×core, seed-trait, and cadence profiles "
    "from history. No daily-play, cuts, RTE, B1Z0, ZLT, rescues, or budget logic is used."
)
...
def build_profiles(trans: pd.DataFrame, min_sample: int = 20, top_n_rule_candidates: int = 500):
    ...
    stream_core = stream_core.sort_values(["lift", "hit_count", "sample_size"], ascending=[False, False, False])
    ...
    seed_trait_core_lift = seed_trait_core_lift.sort_values(["lift", "hit_count", "sample_size"], ascending=[False, False, False])
    ...
    stream_seed_trait_core_lift = stream_seed_trait_core_lift.sort_values(["lift", "hit_count", "sample_size"], ascending=[False, False, False])
    ...
    cadence_core = cadence_core.sort_values(["lift", "hit_count"], ascending=[False, False])
    ...
    common_cols = ["rule_type", "rule", "core_id", "sample_size", "hit_count", "hit_rate_pct", "baseline_rate_pct", "lift_pct_points", "relative_lift_x"]
    ...
    affinity_rule_candidates = affinity_rule_candidates.sort_values(["lift_pct_points", "hit_count", "sample_size"], ascending=[False, False, False]).head(int(top_n_rule_candidates)).reset_index(drop=True)
    ...
    for _, r in latest.iterrows():
        ...
        score["support_notes"] = ""
        sc = stream_core[stream_core["StreamKey"] == stream][["core_id", "lift_pct_points", "hit_rate_pct", "sample_size"]].copy()
        if not sc.empty:
            sc = sc.rename(columns={"lift_pct_points": "stream_lift_pp", "hit_rate_pct": "stream_hit_rate_pct", "sample_size": "stream_sample"})
            score = score.merge(sc, on="core_id", how="left")
            score["affinity_score"] += score["stream_lift_pp"].fillna(0)
        else:
        ...
        for col in seed_trait_cols:
            if col in r.index:
                val = str(r[col])
                tmp = top_trait_lookup[(top_trait_lookup["trait_name"] == col) & (top_trait_lookup["trait_value"].astype(str) == val)][["core_id", "lift_pct_points"]]
                if not tmp.empty:
                    tmp = tmp.rename(columns={"lift_pct_points": f"{col}_lift_pp"})
                    score = score.merge(tmp, on="core_id", how="left")
                    score["affinity_score"] += score[f"{col}_lift_pp"].fillna(0) * 0.25
        ...
        top = score.sort_values(["affinity_score", "stream_lift_pp"], ascending=[False, False]).head(10)
        preview_rows.append(top[["StreamKey", "Seed", "SeedDate", "PlayDateCandidate", "core_id", "affinity_score", "stream_lift_pp", "stream_hit_rate_pct", "stream_sample", "matching_trait_count"]])
    ...
    summary = pd.DataFrame([
        ...
        {"metric": "min_sample_threshold", "value": int(min_sample)},
...
def freeze_exports(outputs: Dict[str, pd.DataFrame]) -> Dict[str, bytes]:
    ...
    frozen["core_affinity_lab_v3_all_outputs.zip"] = bio.getvalue()
    return frozen
...
with st.sidebar:
    ...
    min_sample = st.number_input("Minimum sample size for profile/rule rows", min_value=5, max_value=1000, value=30, step=5)
    top_n_rules = st.number_input("Max rule candidates to export", min_value=50, max_value=5000, value=1000, step=50)
    run_btn = st.button("Build profiles", type="primary", use_container_width=True)

if "v3_outputs" not in st.session_state:
    st.session_state["v3_outputs"] = None
if "v3_export_bytes" not in st.session_state:
    st.session_state["v3_export_bytes"] = None
...
if run_btn:
    ...
    try:
        ...
        outputs = build_profiles(trans, min_sample=int(min_sample), top_n_rule_candidates=int(top_n_rules))
        prog.progress(85, text="Freezing downloads")
        exports = freeze_exports(outputs)
        st.session_state["v3_outputs"] = outputs
        st.session_state["v3_export_bytes"] = exports
...
outputs = st.session_state.get("v3_outputs")
exports = st.session_state.get("v3_export_bytes")
...
st.caption("Candidate rules require later validation before production use.")
...
if isinstance(exports, dict):
    st.download_button(
        "Download ALL v3 outputs ZIP",
        data=exports.get("core_affinity_lab_v3_all_outputs.zip", b""),
        file_name="core_affinity_lab_v3_all_outputs.zip",
