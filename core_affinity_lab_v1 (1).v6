#!/usr/bin/env python3
# Core Affinity Lab v6 — Core Profile Cards + Tie-Breaker Weights
# BUILD: core_affinity_lab_v6__2026-06-17_PROFILE_CARDS_TIEBREAKER_WEIGHTS
...
BUILD_MARKER = "BUILD: core_affinity_lab_v6__2026-06-17_PROFILE_CARDS_TIEBREAKER_WEIGHTS"
ALL_DIGITS = "0123456789"

st.set_page_config(page_title="Core Affinity Lab v6", layout="wide")
st.title("Core Affinity Lab v6 — Core Profile Cards + Tie-Breaker Weights")
st.caption(BUILD_MARKER)
st.info(
    "Lab-only profile builder. Mines stream, core, stream×core, seed-trait, and cadence profiles "
    "from history, then weights signals by sample size/hits/lift stability, builds core-dominance/profile-card tables, and labels stream dominance as tie-breaker context only. No daily-play, cuts, RTE, B1Z0, ZLT, rescues, or budget logic is used."
)
...
def _add_stability_cols(
    ...
    """
    v5 stability layer. This does NOT change mined counts. It only labels/ranks signals so
...
def build_profiles(trans: pd.DataFrame, min_sample: int = 25, top_n_rule_candidates: int = 500, candidate_sample: int = 50, stable_sample: int = 100, strong_sample: int = 250, min_hits_for_signal: int = 5, min_relative_lift_for_signal: float = 1.5, stream_tiebreaker_weight: float = 0.15):
    ...
    # v5 Core dominance tables. These are profile summaries only, not play decisions.
    sc_dom_source = stream_core.copy()
    sc_dom_source["stream_core_rank_by_confidence"] = sc_dom_source.groupby("StreamKey")["confidence_score"].rank(method="first", ascending=False).astype(int)
    sc_dom_source["stream_core_rank_by_weighted_lift"] = sc_dom_source.groupby("StreamKey")["weighted_lift_pct_points"].rank(method="first", ascending=False).astype(int)
    stream_core_dominance = sc_dom_source[
        (sc_dom_source["stream_core_rank_by_confidence"] <= 10)
        & (sc_dom_source["hit_count"] >= int(min_hits_for_signal))
    ].copy()
    stream_core_dominance = stream_core_dominance.sort_values(["StreamKey", "stream_core_rank_by_confidence", "confidence_score"], ascending=[True, True, False])

    top1_by_stream = sc_dom_source[sc_dom_source["stream_core_rank_by_confidence"] == 1].copy()
    top3_by_stream = sc_dom_source[sc_dom_source["stream_core_rank_by_confidence"] <= 3].copy()
    signal_by_core = sc_dom_source[sc_dom_source["meets_signal_floor"]].copy()
    core_dominance_summary = core_profiles[["core_id", "core_rank_by_frequency", "core_hits", "baseline_rate_pct", "members"]].copy()
    core_dominance_summary = core_dominance_summary.merge(
        top1_by_stream.groupby("core_id").size().rename("top1_stream_dominance_count").reset_index(),
        on="core_id", how="left"
    ).merge(
        top3_by_stream.groupby("core_id").size().rename("top3_stream_dominance_count").reset_index(),
        on="core_id", how="left"
    ).merge(
        signal_by_core.groupby("core_id").size().rename("stream_core_signal_count").reset_index(),
        on="core_id", how="left"
    ).merge(
        signal_by_core.groupby("core_id")["confidence_score"].mean().rename("avg_stream_signal_confidence").reset_index(),
        on="core_id", how="left"
    ).merge(
        signal_by_core.groupby("core_id")["weighted_lift_pct_points"].mean().rename("avg_stream_weighted_lift_pp").reset_index(),
        on="core_id", how="left"
    ).fillna({
        "top1_stream_dominance_count": 0,
        "top3_stream_dominance_count": 0,
        "stream_core_signal_count": 0,
        "avg_stream_signal_confidence": 0.0,
        "avg_stream_weighted_lift_pp": 0.0,
    })
    for c in ["top1_stream_dominance_count", "top3_stream_dominance_count", "stream_core_signal_count"]:
        core_dominance_summary[c] = core_dominance_summary[c].astype(int)
    core_dominance_summary["core_dominance_score"] = (
        core_dominance_summary["top1_stream_dominance_count"] * 3.0
        + core_dominance_summary["top3_stream_dominance_count"] * 1.0
        + core_dominance_summary["stream_core_signal_count"] * 0.25
        + core_dominance_summary["avg_stream_signal_confidence"].fillna(0)
    ).round(6)
    core_dominance_summary = core_dominance_summary.sort_values(["core_dominance_score", "top1_stream_dominance_count", "stream_core_signal_count", "core_hits"], ascending=[False, False, False, False]).reset_index(drop=True)
    core_dominance_summary.insert(0, "core_dominance_rank", range(1, len(core_dominance_summary) + 1))

    trait_signal_source = seed_trait_core_lift[seed_trait_core_lift["meets_signal_floor"]].copy()
    trait_core_dominance = trait_signal_source.groupby("core_id").agg(
        seed_trait_signal_count=("rule_type" if "rule_type" in trait_signal_source.columns else "trait_name", "count"),
        avg_seed_trait_confidence=("confidence_score", "mean"),
        avg_seed_trait_weighted_lift_pp=("weighted_lift_pct_points", "mean"),
        max_seed_trait_confidence=("confidence_score", "max"),
    ).reset_index() if not trait_signal_source.empty else pd.DataFrame(columns=["core_id", "seed_trait_signal_count", "avg_seed_trait_confidence", "avg_seed_trait_weighted_lift_pp", "max_seed_trait_confidence"])
    if not trait_core_dominance.empty:
        trait_core_dominance = trait_core_dominance.sort_values(["seed_trait_signal_count", "avg_seed_trait_confidence"], ascending=[False, False])

    # v6 Core profile cards. These are onboarding/profile summaries only.
    # Stream affinity is explicitly marked as tie-breaker context, not a standalone decision rule.
    def _fmt_profile_items(df: pd.DataFrame, label_cols: List[str], max_items: int = 8) -> str:
        if df is None or df.empty:
            return ""
        items = []
        for _, rr in df.head(max_items).iterrows():
            parts = []
            for col in label_cols:
                if col in rr.index:
                    parts.append(f"{col}={rr[col]}")
            if "hit_count" in rr.index and "sample_size" in rr.index and "relative_lift_x" in rr.index:
                parts.append(f"hits={int(rr.get('hit_count', 0))}/{int(rr.get('sample_size', 0))}")
                try:
                    parts.append(f"lift={float(rr.get('relative_lift_x', 0)):.2f}x")
                except Exception:
                    pass
            if "confidence_score" in rr.index:
                try:
                    parts.append(f"conf={float(rr.get('confidence_score', 0)):.3f}")
                except Exception:
                    pass
    ...
    for core in allcores:
        cp = core_profiles[core_profiles["core_id"] == core]
        dom = core_dominance_summary[core_dominance_summary["core_id"] == core]
        trdom = trait_core_dominance[trait_core_dominance["core_id"] == core] if isinstance(trait_core_dominance, pd.DataFrame) and not trait_core_dominance.empty else pd.DataFrame()
        roles = member_role_profile[member_role_profile["core_id"] == core].copy()
        top_streams = stream_core_dominance[stream_core_dominance["core_id"] == core].sort_values(["stream_core_rank_by_confidence", "confidence_score"], ascending=[True, False]).head(8)
        best_seed_traits = seed_trait_core_lift[(seed_trait_core_lift["core_id"] == core) & (seed_trait_core_lift["meets_signal_floor"])].head(8)
        best_stream_seed_traits = stream_seed_trait_core_lift[(stream_seed_trait_core_lift["core_id"] == core) & (stream_seed_trait_core_lift["meets_signal_floor"])].head(8)
        best_cadence = cadence_core[(cadence_core["core_id"] == core) & (cadence_core["meets_signal_floor"])].head(5)
        if not roles.empty:
            role_text = " | ".join([f"{r.member}:{r.matrix_slot}:{int(r.member_hits)} hits:{float(r.member_share_pct):.1f}%" for r in roles.itertuples(index=False)])
        else:
            role_text = ""
        profile_rows.append({
            "core_id": core,
            "members": cp["members"].iloc[0] if not cp.empty else ",".join(core_members(core)),
            "core_frequency_rank": int(cp["core_rank_by_frequency"].iloc[0]) if not cp.empty else None,
            "core_hits": int(cp["core_hits"].iloc[0]) if not cp.empty else 0,
            "baseline_rate_pct": float(cp["baseline_rate_pct"].iloc[0]) if not cp.empty else 0.0,
            "dominance_rank": int(dom["core_dominance_rank"].iloc[0]) if not dom.empty else None,
            "dominance_score": float(dom["core_dominance_score"].iloc[0]) if not dom.empty else 0.0,
            "top1_stream_dominance_count": int(dom["top1_stream_dominance_count"].iloc[0]) if not dom.empty else 0,
            "top3_stream_dominance_count": int(dom["top3_stream_dominance_count"].iloc[0]) if not dom.empty else 0,
            "stream_core_signal_count": int(dom["stream_core_signal_count"].iloc[0]) if not dom.empty else 0,
            "seed_trait_signal_count": int(trdom["seed_trait_signal_count"].iloc[0]) if not trdom.empty and "seed_trait_signal_count" in trdom.columns else 0,
            "member_role_profile": role_text,
            "best_streams_tiebreaker_only": _fmt_profile_items(top_streams, ["StreamKey", "stream_core_rank_by_confidence"], 8),
            "best_seed_traits": _fmt_profile_items(best_seed_traits, ["trait_name", "trait_value"], 8),
            "best_stream_seed_traits": _fmt_profile_items(best_stream_seed_traits, ["StreamKey", "trait_name", "trait_value"], 8),
            "best_cadence_traits": _fmt_profile_items(best_cadence, ["SameCoreGapBucket"], 5),
            "stream_affinity_usage_rule": "TIEBREAKER_ONLY_NOT_PRIMARY_DECISION",
        })
    core_profile_cards = pd.DataFrame(profile_rows).sort_values(["dominance_rank", "core_frequency_rank"], na_position="last").reset_index(drop=True)
    ...
    for _, r in latest.iterrows():
        ...
        if not sc.empty:
            ...
            score["stream_tiebreaker_score"] = score["stream_weighted_lift_pp"].fillna(0) * float(stream_tiebreaker_weight)
            score["affinity_score"] += score["stream_tiebreaker_score"].fillna(0)
        else:
            score["stream_lift_pp"] = 0.0
            score["stream_weighted_lift_pp"] = 0.0
            score["stream_tiebreaker_score"] = 0.0
            score["stream_confidence_score"] = 0.0
            score["stream_hit_rate_pct"] = np.nan
            score["stream_sample"] = np.nan
        score["seed_trait_affinity_score"] = 0.0
        matching_traits = []
        for col in seed_trait_cols:
            if col in r.index:
                val = str(r[col])
                ...
                if not tmp.empty:
                    ...
                    trait_component = score[f"{col}_weighted_lift_pp"].fillna(0) * 0.25
                    score["seed_trait_affinity_score"] += trait_component
                    score["affinity_score"] += trait_component
        ...
        top["stream_affinity_usage_rule"] = "TIEBREAKER_ONLY"
        preview_rows.append(top[["StreamKey", "Seed", "SeedDate", "PlayDateCandidate", "core_id", "affinity_score", "seed_trait_affinity_score", "stream_tiebreaker_score", "stream_lift_pp", "stream_weighted_lift_pp", "stream_confidence_score", "stream_hit_rate_pct", "stream_sample", "matching_trait_count", "stream_affinity_usage_rule"]])
    ...
    summary = pd.DataFrame([
        ...
        {"metric": "stream_tiebreaker_weight", "value": float(stream_tiebreaker_weight)},
        {"metric": "stream_affinity_usage", "value": "TIEBREAKER_ONLY_NOT_PRIMARY_DECISION"},
        ...
        {"metric": "core_dominance_top1_stream_assignments", "value": int(core_dominance_summary["top1_stream_dominance_count"].sum()) if "top1_stream_dominance_count" in core_dominance_summary.columns else 0},
        {"metric": "stream_core_dominance_rows", "value": int(len(stream_core_dominance))},
    ])
    ...
    return {
        ...
        "core_dominance_summary": core_dominance_summary,
        "core_profile_cards": core_profile_cards,
        "stream_core_dominance": stream_core_dominance,
        "trait_core_dominance": trait_core_dominance,
...
def freeze_exports(outputs: Dict[str, pd.DataFrame]) -> Dict[str, bytes]:
    ...
    frozen["core_affinity_lab_v6_all_outputs.zip"] = bio.getvalue()
    return frozen
...
with st.sidebar:
    ...
    min_hits_for_signal = st.number_input("Minimum hits for signal floor", min_value=1, max_value=100, value=10, step=1)
    ...
    stream_tiebreaker_weight = st.number_input("Stream affinity tie-breaker weight", min_value=0.0, max_value=1.0, value=0.15, step=0.05)
    top_n_rules = st.number_input("Max rule candidates to export", min_value=50, max_value=5000, value=1000, step=50)
    st.caption("Tiers: below display min hidden; 25–49 exploratory; 50–99 candidate; 100–249 stable; 250+ strong stable. Stream affinity is tie-breaker context, not the primary daily decision.")
    run_btn = st.button("Build weighted profiles", type="primary", use_container_width=True)

if "v6_outputs" not in st.session_state:
    st.session_state["v6_outputs"] = None
if "v6_export_bytes" not in st.session_state:
    st.session_state["v6_export_bytes"] = None
...
outputs = st.session_state.get("v6_outputs")
exports = st.session_state.get("v6_export_bytes")
...
st.subheader("Core Dominance Summary — stable stream ownership")
st.caption("v6 profile-only table: counts how often each core is the strongest/Top3 stream-core signal. Stream dominance is a tie-breaker prior, not the primary daily decision.")
st.dataframe(outputs["core_dominance_summary"].head(120), use_container_width=True, hide_index=True)

st.subheader("Core Profile Cards — onboarding summaries")
st.caption("One row per core: member roles, best streams, best seed traits, cadence traits, and usage warning. Profile context only.")
st.dataframe(outputs["core_profile_cards"].head(120), use_container_width=True, hide_index=True)

st.subheader("Stream Core Dominance — top cores per stream")
st.caption("Top stream-core profiles by confidence after minimum sample/hit/lift filters.")
st.dataframe(outputs["stream_core_dominance"].head(780), use_container_width=True, hide_index=True)

st.subheader("Trait Core Dominance — seed-trait support by core")
st.dataframe(outputs["trait_core_dominance"].head(120), use_container_width=True, hide_index=True)
...
if isinstance(exports, dict):
    st.download_button(
        "Download ALL v6 outputs ZIP",
        data=exports.get("core_affinity_lab_v6_all_outputs.zip", b""),
        file_name="core_affinity_lab_v6_all_outputs.zip",
    ...
    for fname in [
        ...
        "core_dominance_summary.csv",
        "core_profile_cards.csv",
        "stream_core_dominance.csv",
        "trait_core_dominance.csv",
