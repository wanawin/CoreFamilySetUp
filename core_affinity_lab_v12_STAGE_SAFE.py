#!/usr/bin/env python3
"""
Core Affinity Lab v12 — STAGE SAFE
...
APP_VERSION = "v12"
BUILD_MARKER = "BUILD: core_affinity_lab_v12_STAGE_SAFE__2026-06-17"
DIGITS = "0123456789"
...
st.set_page_config(page_title="Core Affinity Lab v12", layout="wide")
st.title("Core Affinity Lab v12 — Stage-Safe Universal Trait + Member Profiles")
st.caption(BUILD_MARKER)
st.info("Stage-safe lab only. Mines one bounded profile stage at a time to avoid Streamlit Cloud crashes. No daily playlist, no cuts, no RTE, no B1Z0, no ZLT, no rescue logic.")
...
# -----------------------------
with st.sidebar:
    st.header("v12 Controls")
    st.caption("Run one stage at a time. This is intentional to prevent Streamlit Cloud crashes.")
    stage = st.selectbox("Mining stage", [
        "1 - Data audit + base profiles",
        "2 - Core single-trait lift",
        "3 - Member single-trait lift",
        "4 - Core stream+trait lift",
        "5 - Member stream+trait lift",
    ])
...
if st.button("Run selected v12 stage", type="primary", use_container_width=True):
    try:
        ...
        if stage.startswith("1"):
            ...
            df = score_rows(rows, "ActualCore", min_sample, min_hits, min_lift, stable_den, top_n)
            # keep only explicit stream traits or stream-combined context appears as trait=stream=...
        ...
        zip_bytes = make_zip(frames, {"README_v12.txt": "Core Affinity Lab v12 stage-safe output. Run one stage at a time; combine exported CSVs later. Lab only; no playlist logic."})
        freeze_bytes("v12_last_zip", zip_bytes)
        st.success("Stage complete. Download ZIP below.")
        st.download_button("Download v12 selected-stage outputs ZIP", data=zip_bytes, file_name="core_affinity_lab_v12_selected_stage_outputs.zip", mime="application/zip", use_container_width=True)

    except Exception as e:
        st.error("v12 caught the error instead of blank-crashing.")
        st.exception(e)
        st.stop()

if "v12_last_zip" in st.session_state:
    st.download_button("Download previous v12 ZIP again", data=st.session_state["v12_last_zip"], file_name="core_affinity_lab_v12_selected_stage_outputs.zip", mime="application/zip", use_container_width=True)
