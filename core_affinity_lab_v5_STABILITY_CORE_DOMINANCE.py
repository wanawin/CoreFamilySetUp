#!/usr/bin/env python3
# Core Affinity Lab v5 — Stability Weights + Core Dominance
# BUILD: core_affinity_lab_v5__2026-06-17_STABILITY_WEIGHTS_CORE_DOMINANCE
...
BUILD_MARKER = "BUILD: core_affinity_lab_v5__2026-06-17_STABILITY_WEIGHTS_CORE_DOMINANCE"
ALL_DIGITS = "0123456789"

st.set_page_config(page_title="Core Affinity Lab v5", layout="wide")
st.title("Core Affinity Lab v5 — Stability Weights + Core Dominance")
st.caption(BUILD_MARKER)
st.info(
    "Lab-only profile builder. Mines stream, core, stream×core, seed-trait, and cadence profiles "
    "from history, then weights signals by sample size/hits/lift stability and builds core-dominance tables. No daily-play, cuts, RTE, B1Z0, ZLT, rescues, or budget logic is used."
)
...
def freeze_exports(outputs: Dict[str, pd.DataFrame]) -> Dict[str, bytes]:
    ...
    frozen["core_affinity_lab_v5_all_outputs.zip"] = bio.getvalue()
    return frozen
...
if "v5_outputs" not in st.session_state:
    st.session_state["v5_outputs"] = None
if "v5_export_bytes" not in st.session_state:
    st.session_state["v5_export_bytes"] = None
...
if run_btn:
    ...
    try:
        ...
        st.session_state["v5_outputs"] = outputs
        st.session_state["v5_export_bytes"] = exports
...
outputs = st.session_state.get("v5_outputs")
exports = st.session_state.get("v5_export_bytes")
...
st.caption("v5 profile-only table: counts how often each core is the strongest/Top3 stream-core signal. Not a playlist.")
...
if isinstance(exports, dict):
    st.download_button(
        "Download ALL v5 outputs ZIP",
        data=exports.get("core_affinity_lab_v5_all_outputs.zip", b""),
        file_name="core_affinity_lab_v5_all_outputs.zip",
