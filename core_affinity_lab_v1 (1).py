#!/usr/bin/env python3
"""
Core Affinity Lab v39 — Trait Group Split Separator Engine Progress Safe
...
APP_VERSION = "v39"
BUILD_MARKER = "BUILD: core_affinity_lab_v39_TRAIT_GROUP_SPLIT_SEPARATOR_ENGINE_PROGRESS_SAFE__2026-06-19"
DEPLOY_FILENAME_NOTE = "For Streamlit Cloud deployment, this file may be renamed to: core_affinity_lab_v1 (1).py"

st.set_page_config(page_title="Core Affinity Lab v39", layout="wide")
st.title("Core Affinity Lab v39 — Trait Group Split Separator Engine Progress Safe")
st.caption(BUILD_MARKER)
st.info(
    "Group first, split only collision groups, preserve seed lineage. v39 fixes gc-module shadowing and adds visible progress. "
...
try:
    ...
    run = st.button("Run selected v39 stage and prepare frozen ZIP", type="primary")
    if run:
        t0 = time.time()
        frames: Dict[str, pd.DataFrame] = {"summary_basic.csv": basic}
        texts: Dict[str, str] = {"README_v39.txt": f"{BUILD_MARKER}\n{DEPLOY_FILENAME_NOTE}\nStage: {stage}\nDiscovery lab only. Every processed seed row is assigned exactly once in seed_lineage_audit.csv.\n"}
        if stage.startswith("1"):
            group_summary_rows = []
            stage_prog = st.progress(0.0)
            stage_status = st.empty()
            group_cols_present = [group_name for group_name in GROUP_COLS if group_name in ledger.columns]
            for idx, group_name in enumerate(group_cols_present, start=1):
                stage_status.info(f"Auditing group column {idx}/{len(group_cols_present)}: {group_name}")
                stage_prog.progress(idx / max(1, len(group_cols_present)))
                gs = ledger.groupby(group_name, dropna=False).size().reset_index(name="seed_rows")
                group_summary_rows.append({"group_col": group_name, "distinct_groups": len(gs), "largest_group": int(gs["seed_rows"].max()) if not gs.empty else 0, "total_seed_rows": int(gs["seed_rows"].sum()) if not gs.empty else 0})
                frames[f"audit_{group_name}_group_sizes_top500.csv"] = gs.sort_values("seed_rows", ascending=False).head(500)
        ...
        st.success(f"Frozen v39 ZIP prepared. Size: {zip_mb:.2f} MB. Elapsed: {manifest['elapsed_seconds']} sec.")
    ...
    if st.session_state.get("v38_package"):
        st.download_button(
            "Download v39 grouped separator outputs ZIP",
            st.session_state["v38_package"],
            file_name=f"core_affinity_lab_v39_{stage.split()[0]}_{group_col}_start_{int(start_group)}.zip",
            mime="application/zip",
        )

except Exception as e:
    st.error("v39 failed. Full exception below.")
    st.exception(e)
