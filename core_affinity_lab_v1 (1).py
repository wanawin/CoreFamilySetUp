#!/usr/bin/env python3
"""
Core Affinity Lab v19 — COMPOUND RELAXED THRESHOLDS + DEEPER MINING CONTROLS
...
APP_VERSION = "v19"
BUILD_MARKER = "BUILD: core_affinity_lab_v19_COMPOUND_RELAXED_THRESHOLDS_DEEPER_MINING__2026-06-17"
DIGITS = "0123456789"
...
with st.sidebar:
    ...
    st.markdown("---")
    st.caption("Compound-only discovery thresholds. These are intentionally lower than single-trait thresholds because stacked traits shrink sample size. Label as exploratory until validated.")
...
if st.button("Run selected v19 stage", type="primary", use_container_width=True):
    ...
    try:
        ...
        zip_bytes = make_zip(frames, {"README_v19.txt": "Core Affinity Lab v19 compound-separator output. Compound stages use relaxed discovery thresholds and deeper bounded mining controls. Discovery only; no playlist logic and not production play rules."})
        freeze_bytes("v19_last_zip", zip_bytes)
        st.success("Stage complete. Download ZIP below.")
        st.download_button("Download v19 selected-stage outputs ZIP", data=zip_bytes, file_name="core_affinity_lab_v19_selected_stage_outputs.zip", mime="application/zip", use_container_width=True)

    except Exception as e:
        st.error("v19 caught the error instead of blank-crashing.")
        st.exception(e)
        st.stop()

if "v17_last_zip" in st.session_state:
    st.download_button("Download previous v17 ZIP again", data=st.session_state["v17_last_zip"], file_name="core_affinity_lab_v19_selected_stage_outputs.zip", mime="application/zip", use_container_width=True)
