...
APP_VERSION = "v25"
BUILD_MARKER = "BUILD: core_affinity_lab_v25_FAST_PROFILE_DECOMPOSITION_FIX__2026-06-18"
DEPLOY_FILENAME_NOTE = "For Streamlit Cloud deployment, this file may be renamed to: core_affinity_lab_v1 (1).py"

st.set_page_config(page_title="Core Affinity Lab v25", layout="wide")
st.title("Core Affinity Lab v25 — Fast Profile Decomposition Fix")
st.caption(BUILD_MARKER)
st.info(
    "Coverage-first lab: build multiple profiles per core/member, then mine separators only for collisions. v25 uses fast indexed matching so a single core cannot run forever. "
...
with st.sidebar:
    ...
    st.caption("v25 runs decomposition in chunks with fast indexed profile matching. Profile discovery is coverage-first; precision is reported for separator refinement, not used to block initial buckets.")
...
try:
    ...
    texts = {"README_v25.txt": f"{BUILD_MARKER}\n{DEPLOY_FILENAME_NOTE}\nStage run: {stage}\nDiscovery lab only. v25 fixes the v24 stall by using indexed trait matching, greedy bounded refinement, and per-target evaluation caps.\n"}
    ...
    st.download_button(
        "Download selected v25 stage outputs ZIP",
        package,
        file_name=f"core_affinity_lab_v25_stage_{stage.split()[0]}_chunk_{int(target_start)}.zip",
        mime="application/zip",
    )

except Exception as e:
    st.error("v25 stage failed. Full traceback below.")
    st.exception(e)
