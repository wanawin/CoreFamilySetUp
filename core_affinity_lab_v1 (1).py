#!/usr/bin/env python3
"""
Core Affinity Lab v36 — Separator Discovery Audit Miner
...
APP_VERSION = "v36"
BUILD_MARKER = "BUILD: core_affinity_lab_v36_SEPARATOR_DISCOVERY_AUDIT__2026-06-18"
DEPLOY_FILENAME_NOTE = "For Streamlit Cloud deployment, this file may be renamed to: core_affinity_lab_v1 (1).py"

st.set_page_config(page_title="Core Affinity Lab v36", layout="wide")
st.title("Core Affinity Lab v36 — Separator Discovery Audit Miner")
st.caption(BUILD_MARKER)
st.info(
    "Upload clean history plus all v30 Stage-2 core-profile ZIP chunks. "
    "v36 merges v30 coverage buckets globally, deduplicates overlaps, then mines INSTRUMENTED CAPPED beam-search separators: target rows vs sibling rows inside each broad bucket. "
...
def mine_profile_separators(
    ...
    """v36 discovery-audit instrumented/capped beam-search collision separator miner.

    v34 proved the beam idea but could kill Streamlit on profile 1. v36 is intentionally
    more conservative: it reports candidate counts, caps evaluations, prunes local output,
    and keeps only the best separator rows per profile. This is a diagnostic/proving build,
    not the final deepest miner.
    """
    ...
    core_values = rows["core_id"].astype(str).tolist()
    ...
    discovery_rows: List[dict] = []  # v36: includes passed AND failed candidates for threshold diagnosis
    total = max(1, len(selected))

    # Hard safety caps for Streamlit Community Cloud.
    beam_width = max(3, min(int(top_per_profile), 15))
    max_depth = max(1, min(3, int(refine_depth)))
    level1_pool = max(10, min(int(candidate_pool), 90))
    addon_pool = max(10, min(int(candidate_pool), 45))
    max_evals_per_profile = 4500
    max_seconds_per_profile = 35
    max_output_per_profile = max(20, min(60, beam_width * 4))
    max_global_output = 5000

    def is_redundant_with_stack(trait: str, stack: Tuple[str, ...]) -> bool:
        base = set(stack)
        if trait in base:
            return True
    ...
    def score_matched(target_set: Set[int], sibling_set: Set[int], matched: Set[int], total_target_hits: int, base_precision: float, profile_target_total: int) -> dict:
        ...
        score = (
            max(0.0, precision_gain) * 1200.0
            + min(lift, 100.0) * 12.0
            + min(odds_ratio, 200.0) * 3.0
            + profile_coverage * 220.0
            + global_coverage * 50.0
            + target_hits * 0.35
            + max(0, hit_margin) * 0.04
        )
        return {
            "sample": sample,
    ...
    def row_from_candidate(pr, target, base_stack, sep_stack, bucket, bucket_target, bucket_sibling, stats, base_precision, base_target_coverage):
        return {
            "profile_row_num": int(pr.get("profile_row_num", 0)),
            "target": target,
            "profile_id": pr.get("profile_id", ""),
            "profile_num": pr.get("profile_num", ""),
            "base_stack": " && ".join(base_stack),
            "separator_stack": " && ".join(sep_stack),
            "refined_stack": " && ".join(base_stack + sep_stack),
            "separator_depth": len(sep_stack),
            ...
            "candidate_rank_basis": "v35_capped_beam: precision_gain + lift + odds_ratio + retained_coverage",
            "needs_validation": True,
            "production_status": "DISCOVERY_SEPARATOR_NOT_PRODUCTION_RULE",
        }

    def row_from_discovery_candidate(pr, target, base_stack, sep_stack, bucket, bucket_target, bucket_sibling, stats, base_precision, base_target_coverage, phase, accepted, reject_reason):
        stats = stats or {
            "sample": 0, "target_hits": 0, "sibling_hits": 0, "precision": 0.0,
            "precision_gain": 0.0, "profile_coverage": 0.0, "global_target_coverage": 0.0,
            "target_rate_in_bucket": 0.0, "sibling_rate_in_bucket": 0.0,
            "target_vs_sibling_lift": 0.0, "odds_ratio": 0.0, "hit_margin": 0, "score": 0.0,
        }
        return {
            "profile_row_num": int(pr.get("profile_row_num", 0)),
            "target": target,
            ...
            "profile_num": pr.get("profile_num", ""),
            "phase": phase,
            "accepted_separator": bool(accepted),
            ...
            "base_precision": round(base_precision, 6),
            "base_target_coverage": round(base_target_coverage, 6),
            "sample": int(stats.get("sample", 0)),
            "target_hits": int(stats.get("target_hits", 0)),
            "sibling_hits": int(stats.get("sibling_hits", 0)),
            "precision": round(float(stats.get("precision", 0.0)), 6),
            "precision_gain": round(float(stats.get("precision_gain", 0.0)), 6),
            "profile_coverage": round(float(stats.get("profile_coverage", 0.0)), 6),
            "global_target_coverage": round(float(stats.get("global_target_coverage", 0.0)), 6),
            "target_rate_in_bucket": round(float(stats.get("target_rate_in_bucket", 0.0)), 6),
            "sibling_rate_in_bucket": round(float(stats.get("sibling_rate_in_bucket", 0.0)), 6),
            "target_vs_sibling_lift": round(float(stats.get("target_vs_sibling_lift", 0.0)), 6),
            "odds_ratio": round(float(stats.get("odds_ratio", 0.0)), 6),
            "hit_margin": int(stats.get("hit_margin", 0)),
            "score": round(float(stats.get("score", 0.0)), 6),
            "candidate_rank_basis": "v36_discovery_audit: includes failed candidates; sort by score/precision_gain/lift/coverage",
            "production_status": "AUDIT_ONLY_NOT_PRODUCTION_RULE",
        }
    ...
    for n, (_, pr) in enumerate(selected.iterrows(), start=1):
        ...
        if status is not None:
            status.info(f"v36 discovery-audit capped beam: profile {n}/{total} — core {target} profile {pr.get('profile_num','')} — building bucket")
        ...
        audit = {
            ...
            "beam_width_effective": beam_width,
            "max_depth_effective": max_depth,
            "level1_pool_effective": level1_pool,
            "addon_pool_effective": addon_pool,
            "max_evals_per_profile": max_evals_per_profile,
            "stopped_reason": "",
            "note": "v36 discovery-audit capped/instrumented beam search; profile chunks should start small and scale only after download success.",
        }
        if len(bucket_target) < min_target_hits or not bucket:
            audit["stopped_reason"] = "not_enough_target_hits_in_bucket"
            audit_rows.append(audit)
            continue

        target_counts = count_traits(rows, bucket_target)
        sibling_counts = count_traits(rows, bucket_sibling) if bucket_sibling else Counter()
        pre = []
        for trait, th in target_counts.items():
            if is_redundant_with_stack(trait, base_stack):
                continue
            sh = sibling_counts.get(trait, 0)
            tr = th / max(1, len(bucket_target))
            sr = sh / max(1, len(bucket_sibling))
            pre_score = (tr + eps) / (sr + eps) * 10.0 + th * 0.08 - sh * 0.0002
            pre.append((pre_score, trait, th, sh))
        pre.sort(reverse=True)
        level1_traits = [t for _s, t, _th, _sh in pre[:level1_pool]]

        if status is not None:
            status.info(f"v36 discovery-audit capped beam: profile {n}/{total} — target rows {len(bucket_target):,}, siblings {len(bucket_sibling):,}, level1 traits {len(level1_traits):,}")

        local_candidates = []
        local_discovery = []  # v36: keep best failed and passed candidates for diagnosis
        beam = []
        seen = set()
        total_examined = 0
        stopped_reason = "completed"

        for trait in level1_traits:
            if time.time() - profile_t0 > max_seconds_per_profile:
                stopped_reason = "profile_time_cap_level1"
                break
            total_examined += 1
            if total_examined > max_evals_per_profile:
                stopped_reason = "profile_eval_cap_level1"
                break
            matched = bucket & trait_index.get(trait, set())
            stats = score_matched(bucket_target, bucket_sibling, matched, total_target_hits, base_precision, len(bucket_target))
            sep_stack = (trait,)
            accepted = True
            reject_reason = "accepted"
            if not stats:
                accepted = False
                reject_reason = "no_matched_sample"
            elif stats["target_hits"] < min_target_hits:
                accepted = False
                reject_reason = "below_min_target_hits"
            elif stats["target_vs_sibling_lift"] <= 1.01 and stats["precision_gain"] <= 0:
                accepted = False
                reject_reason = "weak_lift_and_no_precision_gain"
            if stats:
                local_discovery.append(row_from_discovery_candidate(pr, target, base_stack, sep_stack, bucket, bucket_target, bucket_sibling, stats, base_precision, base_target_coverage, "level1", accepted, reject_reason))
            if not accepted:
                continue
            seen.add(sep_stack)
            cand = (stats["score"], sep_stack, matched, stats)
            beam.append(cand)
        ...
        for depth in range(2, max_depth + 1):
            if not current_beam or stopped_reason != "completed":
                break
            next_candidates = []
            for _score, sep_stack, matched_prev, stats_prev in current_beam:
                if time.time() - profile_t0 > max_seconds_per_profile:
                    stopped_reason = f"profile_time_cap_depth_{depth}"
                    break
                if total_examined > max_evals_per_profile:
                    stopped_reason = f"profile_eval_cap_depth_{depth}"
                    break
                target_after = matched_prev & bucket_target
                if len(target_after) < min_target_hits:
                    continue
                addon_counts = count_traits(rows, target_after)
                for trait, _cnt in addon_counts.most_common(addon_pool):
                    if trait in sep_stack or is_redundant_with_stack(trait, base_stack + sep_stack):
                        continue
                    total_examined += 1
                    if total_examined > max_evals_per_profile:
                        stopped_reason = f"profile_eval_cap_depth_{depth}"
                        break
                    new_sep_stack = tuple(sorted(sep_stack + (trait,)))
                    if new_sep_stack in seen:
                        continue
                    seen.add(new_sep_stack)
                    matched = matched_prev & trait_index.get(trait, set())
                    stats = score_matched(bucket_target, bucket_sibling, matched, total_target_hits, base_precision, len(bucket_target))
                    accepted = True
                    reject_reason = "accepted"
                    if not stats:
                        ...
                        accepted = False
                        reject_reason = "below_min_target_hits"
                    elif stats["precision"] + 1e-9 < stats_prev["precision"] and stats["target_vs_sibling_lift"] <= stats_prev["target_vs_sibling_lift"]:
                        accepted = False
                        reject_reason = "worse_precision_without_lift_gain"
                    elif (stats["precision_gain"] <= 0 and stats["target_vs_sibling_lift"] <= 1.05 and stats["hit_margin"] <= stats_prev["hit_margin"]):
                        accepted = False
                        reject_reason = "weak_compound_discriminator"
                    if stats:
                        local_discovery.append(row_from_discovery_candidate(pr, target, base_stack, new_sep_stack, bucket, bucket_target, bucket_sibling, stats, base_precision, base_target_coverage, f"depth{depth}", accepted, reject_reason))
                    if not accepted:
                        continue
                    cand = (stats["score"], new_sep_stack, matched, stats)
                    next_candidates.append(cand)
                    local_candidates.append(cand)
                    if len(local_candidates) > max_output_per_profile * 5:
                        local_candidates = sorted(local_candidates, key=lambda x: x[0], reverse=True)[:max_output_per_profile * 3]
                if stopped_reason != "completed":
                    break
            current_beam = sorted(next_candidates, key=lambda x: x[0], reverse=True)[:beam_width]

        local_candidates = sorted(local_candidates, key=lambda x: x[0], reverse=True)[:max_output_per_profile]
        # v36: export the best failed candidates too. This is the main diagnostic output.
        if local_discovery:
            local_discovery = sorted(local_discovery, key=lambda r: (float(r.get("score", 0)), float(r.get("precision_gain", 0)), float(r.get("target_vs_sibling_lift", 0)), float(r.get("profile_coverage", 0))), reverse=True)[:max_output_per_profile * 4]
            discovery_rows.extend(local_discovery)
        for _score, sep_stack, matched, stats in local_candidates:
            sep_rows.append(row_from_candidate(pr, target, base_stack, sep_stack, bucket, bucket_target, bucket_sibling, stats, base_precision, base_target_coverage))
        if len(sep_rows) > max_global_output:
            sep_rows = sorted(sep_rows, key=lambda r: r.get("score", 0), reverse=True)[:max_global_output]
            stopped_reason = "global_output_cap_reached"

        audit.update({
            "level1_traits_considered": len(level1_traits),
            "total_candidates_examined": total_examined,
            "local_candidates_kept": len(local_candidates),
            "discovery_candidates_exported_for_profile": len(local_discovery),
            "separator_rows_generated_for_profile": len(local_candidates),
            "profile_elapsed_seconds": round(time.time() - profile_t0, 3),
            "stopped_reason": stopped_reason,
        })
        audit_rows.append(audit)
        gc.collect()
        if stopped_reason == "global_output_cap_reached":
            break

    if progress is not None:
        progress.progress(1.0)
    if status is not None:
        status.success(f"Finished v36 discovery-audit capped beam mining for {len(selected)} profiles in {time.time()-t0:.2f} seconds.")
    seps = pd.DataFrame(sep_rows)
    if not seps.empty:
        seps = seps.sort_values(["score", "separator_depth", "precision_gain", "target_vs_sibling_lift", "precision", "profile_coverage", "target_hits"], ascending=[False, False, False, False, False, False, False]).reset_index(drop=True)
    audit = pd.DataFrame(audit_rows)
    discovery = pd.DataFrame(discovery_rows)
    if not discovery.empty:
        discovery = discovery.sort_values(["score", "precision_gain", "target_vs_sibling_lift", "precision", "profile_coverage", "target_hits"], ascending=[False, False, False, False, False, False]).reset_index(drop=True)
    return seps, audit, discovery
...
# -------------------------
# v34 Arrow / Streamlit display safety
# -------------------------
...
with st.sidebar:
    ...
    profile_chunk_size = st.slider("Profile chunk size", 1, 80, 10, 1)
    ...
    candidate_pool = st.slider("Separator candidate trait pool", 25, 220, 80, 5)
    beam_width = st.slider("Beam width per profile", 3, 30, 10, 1)
    refine_depth = st.slider("Beam-search separator max depth", 1, 3, 2)
    export_top_n = st.slider("CSV export top N rows", 100, 5000, 1000, 100)
    st.caption("v36 is manifest-only by default. It uses bounded beam-search separator stacking against target-vs-sibling rows; outputs are discovery-only.")
...
if "v36_package" not in st.session_state:
    st.session_state["v36_package"] = None
    st.session_state["v36_manifest"] = None
    st.session_state["v36_filename"] = None

run = st.button("Run selected v36 stage and prepare frozen ZIP", type="primary")
...
if run:
    ...
    try:
        ...
        if stage.startswith("1"):
            ...
            frames["v36_zip_manifest.csv"] = zip_manifest
            ...
            status = st.empty()
            mining_t0 = time.time()
            seps, audit, discovery = mine_profile_separators(
                rows=rows,
                profiles=profiles,
                profile_start=int(profile_start),
                profile_chunk_size=int(profile_chunk_size),
                ...
                top_per_profile=int(beam_width),
            ...
            frames["separator_discovery_audit_candidates.csv"] = discovery.head(export_top_n)
            metrics.update({
        ...
        texts["README_v36.txt"] = (
            ...
            "v36 consumes v30 coverage profiles and mines capped/instrumented discriminative beam-search separator stacks using target-vs-sibling frequency, lift, odds ratio, and precision gain. Hard caps prevent profile-1 candidate explosion.\n"
        )
        package = zip_frames(frames, texts)
        filename = f"core_affinity_lab_v36_stage_{stage.split()[0]}_profiles_{int(profile_start)}.zip"
        st.session_state["v36_package"] = package
        st.session_state["v36_manifest"] = manifest
        st.session_state["v36_filename"] = filename
        # drop big frames before rendering UI
        del frames, profiles, residuals, zip_manifest, package
        gc.collect()
        st.success("v36 frozen ZIP prepared. Download below. No rerun/rebuild is needed.")

    except Exception as e:
        st.error("v36 stage failed. Full traceback below.")
        st.exception(e)

if st.session_state.get("v36_manifest") is not None:
    st.subheader("Frozen package manifest")
    st.table(safe_display_df(st.session_state["v36_manifest"]))

if st.session_state.get("v36_package") is not None:
    st.download_button(
        "Download frozen v36 stage outputs ZIP",
        st.session_state["v36_package"],
        file_name=st.session_state.get("v36_filename", "core_affinity_lab_v36_outputs.zip"),
        mime="application/zip",
        key="v36_download_button",
    )
