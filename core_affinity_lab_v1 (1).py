#!/usr/bin/env python3
"""
Core Affinity Lab v32 — Global Core Separator Miner
...
APP_VERSION = "v32"
BUILD_MARKER = "BUILD: core_affinity_lab_v32_ARROW_TYPE_SAFE_STAGE_HANDOFF__2026-06-18"
DEPLOY_FILENAME_NOTE = "For Streamlit Cloud deployment, this file may be renamed to: core_affinity_lab_v1 (1).py"

st.set_page_config(page_title="Core Affinity Lab v32", layout="wide")
st.title("Core Affinity Lab v32 — Global Core Separator Miner")
st.caption(BUILD_MARKER)
st.info(
    "Upload clean history plus all v30 Stage-2 core-profile ZIP chunks. "
    "v32 merges v30 coverage buckets globally, deduplicates overlaps, then mines separators inside each broad bucket. "
...
# -------------------------
# v32 Arrow / Streamlit display safety
# -------------------------
...
with st.sidebar:
    ...
    st.caption("v32 is manifest-only by default. It mines separator candidates inside v30 coverage buckets; outputs are discovery-only.")
...
if "v32_package" not in st.session_state:
    st.session_state["v32_package"] = None
    st.session_state["v32_manifest"] = None
    st.session_state["v32_filename"] = None

run = st.button("Run selected v32 stage and prepare frozen ZIP", type="primary")
...
if run:
    ...
    try:
        ...
        if stage.startswith("1"):
            ...
            frames["v32_zip_manifest.csv"] = zip_manifest
        ...
        texts["README_v32.txt"] = (
            ...
            "v32 consumes v30 coverage profiles and mines separator traits inside profile collision buckets.\n"
        )
        package = zip_frames(frames, texts)
        filename = f"core_affinity_lab_v32_stage_{stage.split()[0]}_profiles_{int(profile_start)}.zip"
        st.session_state["v32_package"] = package
        st.session_state["v32_manifest"] = manifest
        st.session_state["v32_filename"] = filename
        # drop big frames before rendering UI
        del frames, profiles, residuals, zip_manifest, package
        gc.collect()
        st.success("v32 frozen ZIP prepared. Download below. No rerun/rebuild is needed.")

    except Exception as e:
        st.error("v32 stage failed. Full traceback below.")
        st.exception(e)

if st.session_state.get("v32_manifest") is not None:
    st.subheader("Frozen package manifest")
    st.table(safe_display_df(st.session_state["v32_manifest"]))

if st.session_state.get("v32_package") is not None:
    st.download_button(
        "Download frozen v32 stage outputs ZIP",
        st.session_state["v32_package"],
        file_name=st.session_state.get("v32_filename", "core_affinity_lab_v32_outputs.zip"),
        mime="application/zip",
        key="v32_download_button",
    )
