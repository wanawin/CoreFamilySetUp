#!/usr/bin/env python3
"""
Core Affinity Lab v8 — Universal Trait + Member Profiles MEMORY SAFE
...
APP_VERSION = "v8"
BUILD_MARKER = "BUILD: core_affinity_lab_v8_UNIVERSAL_TRAIT_MEMBER_PROFILES_MEMORY_SAFE__2026-06-17"

st.set_page_config(page_title="Core Affinity Lab v8", layout="wide")
st.title("Core Affinity Lab v8 — Universal Trait + Member Profiles MEMORY SAFE")
st.caption(BUILD_MARKER)
st.info(
    "Lab only. Mines universal seed traits across all 120 AABC cores and all 360 AABC members. "
    "No daily playlist, no cuts, no RTE, no B1Z0, no ZLT, no rescue logic. v8 uses counter-based mining to avoid Streamlit memory crashes."
)
...
def _freeze_outputs(outputs: Dict[str, pd.DataFrame], texts: Dict[str, str] | None = None) -> bytes:
    ...
    data = bio.getvalue()
    st.session_state["v8_outputs"] = outputs
    st.session_state["v8_texts"] = texts
    st.session_state["v8_zip_bytes"] = data
    return data


def _download_df(label: str, key: str, filename: str):
    outputs = st.session_state.get("v8_outputs", {})
    df = outputs.get(key)
    if isinstance(df, pd.DataFrame):
        st.download_button(
            label,
...
# Sidebar controls
# -----------------------------
st.sidebar.header("v8 Controls")
st.sidebar.caption("v8 memory-safe: mines with counters instead of giant exploded tables.")
...
run_btn = st.button("Run v8 universal trait/member profiler", type="primary", use_container_width=True)
...
if run_btn:
    ...
    zip_bytes = _freeze_outputs(outputs, {"README_v8.txt": readme})
    progress.progress(100, text="v8 complete.")

if "v8_outputs" not in st.session_state:
    st.stop()

outputs = st.session_state["v8_outputs"]
st.success("v8 outputs ready. Download buttons use frozen session bytes.")
...
st.download_button(
    "Download all v8 outputs ZIP",
    data=st.session_state.get("v8_zip_bytes", b""),
    file_name="core_affinity_lab_v8_all_outputs.zip",
    mime="application/zip",
    key="download_v8_all_zip",
    use_container_width=True,
)
for key in [
