#!/usr/bin/env python3
"""
Core Affinity Lab v16 — COMPOUND SEPARATOR MINER
...
APP_VERSION = "v16"
BUILD_MARKER = "BUILD: core_affinity_lab_v16_COMPOUND_SEPARATOR_MINER__2026-06-17"
DIGITS = "0123456789"
...
st.set_page_config(page_title="Core Affinity Lab v16", layout="wide")
st.title("Core Affinity Lab v16 — Compound Separator Miner")
st.caption(BUILD_MARKER)
st.info("v16 keeps the v15 universal trait universe and adds bounded 2-trait / 3-trait compound separator mining for ALL 120 cores and ALL 360 members.")
...
if st.button("Run selected v15 stage", type="primary", use_container_width=True):
    try:
        ...
        zip_bytes = make_zip(frames, {"README_v16.txt": "Core Affinity Lab v16 compound-separator output. Run one stage at a time; combine exported CSVs later. Lab only; no playlist logic."})
        freeze_bytes("v16_last_zip", zip_bytes)
        st.success("Stage complete. Download ZIP below.")
        st.download_button("Download v16 selected-stage outputs ZIP", data=zip_bytes, file_name="core_affinity_lab_v16_selected_stage_outputs.zip", mime="application/zip", use_container_width=True)

    except Exception as e:
        st.error("v16 caught the error instead of blank-crashing.")
        st.exception(e)
        st.stop()

if "v16_last_zip" in st.session_state:
    st.download_button("Download previous v16 ZIP again", data=st.session_state["v16_last_zip"], file_name="core_affinity_lab_v16_selected_stage_outputs.zip", mime="application/zip", use_container_width=True)
