#!/usr/bin/env python3
"""
Core Affinity Lab v23 — Chunked Profile Decomposition Engine
...
APP_VERSION = "v23"
BUILD_MARKER = "BUILD: core_affinity_lab_v23_PROFILE_DECOMPOSITION_CHUNKED_SAFE__2026-06-18"
DEPLOY_FILENAME_NOTE = "For Streamlit Cloud deployment, this file may be renamed to: core_affinity_lab_v1 (1).py"

st.set_page_config(page_title="Core Affinity Lab v23", layout="wide")
st.title("Core Affinity Lab v23 — Chunked Profile Decomposition Engine")
...
with st.sidebar:
    ...
    st.caption("v23 runs decomposition in chunks to avoid Streamlit Cloud resets. Run chunks sequentially and combine CSVs later.")
...
try:
    ...
    texts = {"README_v23.txt": f"{BUILD_MARKER}\n{DEPLOY_FILENAME_NOTE}\nStage run: {stage}\nDiscovery lab only.\n"}
    ...
    st.download_button(
        "Download selected v23 stage outputs ZIP",
        package,
        file_name=f"core_affinity_lab_v23_stage_{stage.split()[0]}_chunk_{int(target_start)}.zip",
        mime="application/zip",
    )

except Exception as e:
    st.error("v23 stage failed. Full traceback below.")
    st.exception(e)
