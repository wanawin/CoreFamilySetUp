#!/usr/bin/env python3
"""
Core Affinity Lab v25 — Fast Profile Decomposition Fix
...
APP_VERSION = "v30"
BUILD_MARKER = "BUILD: core_affinity_lab_v30_DOWNLOAD_LIFECYCLE_FINAL__2026-06-18"
DEPLOY_FILENAME_NOTE = "For Streamlit Cloud deployment, this file may be renamed to: core_affinity_lab_v1 (1).py"

st.set_page_config(page_title="Core Affinity Lab v30", layout="wide")
st.title("Core Affinity Lab v30 — Download Lifecycle Final")
st.caption(BUILD_MARKER)
st.info(
    "Coverage-first lab: build multiple profiles per core/member, then mine separators only for collisions. v30 uses early frozen-download exit, manifest-only UI, explicit cleanup, and no rebuild on download. "
...
# -------------------------
# v25 optimized decomposition override
# -------------------------
def _build_trait_index(rows: pd.DataFrame) -> Dict[str, set]:
    idx = defaultdict(set)
...
def _fast_stats(rows: pd.DataFrame, level: str, target: str, matched_all: set, remaining_target_idxs: set, target_idxs_all: set) -> Dict[str, float]:
    if not matched_all:
        return {"sample":0,"target_hits":0,"target_hits_all":0,"sibling_hits":0,"precision":0.0,"target_coverage":0.0,"remaining_coverage":0.0}
    target_hits_all = len(matched_all & target_idxs_all)
    target_hits_remaining = len(matched_all & remaining_target_idxs)
    sample = len(matched_all)
    sibling_hits = sample - target_hits_all
    total_target = len(target_idxs_all)
    return {
...
def _count_traits_for_idxs(rows: pd.DataFrame, idxs: Iterable[int]) -> Counter:
    c = Counter()
    trait_lists = rows["trait_list"].tolist()
    for i in idxs:
        c.update(trait_lists[i])
    return c


def build_profiles(rows: pd.DataFrame, level: str, cfg: ProfileCfg, target_limit: int | None = None, target_start: int = 1, target_chunk_size: int | None = None, progress=None, status=None) -> Tuple[pd.DataFrame, pd.DataFrame]:
    """v25 fast coverage-first decomposition.

    Important difference from v24:
    - Builds a trait->row index once.
    - Scores stacks by set intersections instead of repeatedly scanning all rows.
    - Greedy bounded refinement only considers a small addon pool.
    - Hard evaluation caps prevent one core/member from running forever.
    """
    if rows.empty:
        return pd.DataFrame(), pd.DataFrame()
    ...
    trait_index = _build_trait_index(rows)
    target_values = rows["core_id"].tolist() if level == "core" else rows["member"].tolist()
    target_index = defaultdict(set)
    for i, val in enumerate(target_values):
        target_index[val].add(i)

    profile_rows = []
    residual_rows = []
    total_targets = max(1, len(targets))

    # Hard caps. These are intentionally conservative for Streamlit Cloud.
    refine_pool_cap = min(cfg.top_trait_pool, 30)
    single_pool_cap = min(cfg.top_trait_pool, 120)
    max_evals_per_profile = 900
    max_seconds_per_target = 90
    ...
    for target_i, target in enumerate(targets, start=1):
        ...
        target_idxs_all = set(target_index.get(target, set()))
        remaining = set(target_idxs_all)
        total_target = len(target_idxs_all)
        target_eval_count = 0
        timed_out = False
        ...
        for pnum in range(1, cfg.max_profiles_per_target + 1):
            if len(remaining) < cfg.min_target_hits:
                break
            if time.time() - target_t0 > max_seconds_per_target:
                timed_out = True
                break

            counts = _count_traits_for_idxs(rows, remaining)
            pool = [t for t, n in counts.most_common(single_pool_cap) if n >= cfg.min_target_hits]
            best_stack = None
            best_stats = None
            best_match_all = set()
            best_score = -1.0
            ...
            for t in pool:
                target_eval_count += 1
                matched_all = trait_index.get(t, set())
                stats = _fast_stats(rows, level, target, matched_all, remaining, target_idxs_all)
                if stats["target_hits"] < cfg.min_target_hits:
                    continue
                margin = stats["target_hits_all"] - stats["sibling_hits"]
                score = (stats["target_hits"] * 100.0) + (stats["remaining_coverage"] * 100.0) + (stats["precision"] * 2.0) + max(0, margin) * 0.02
                if score > best_score:
                    best_score = score
                    best_stack = (t,)
                    best_stats = stats
                    best_match_all = set(matched_all)
            ...
            current = best_stack
            current_stats = best_stats
            current_match_all = best_match_all

            # Greedy refinement: only add if coverage remains meaningful and precision/margin improve.
            for depth in range(2, cfg.max_stack_depth + 1):
                if target_eval_count > max_evals_per_profile * pnum:
                    break
                if time.time() - target_t0 > max_seconds_per_target:
                    timed_out = True
                    break
                matched_remaining = current_match_all & remaining
                addon_counts = _count_traits_for_idxs(rows, matched_remaining)
                add_best = None
                add_stats = None
                add_match_all = None
                add_score = -1.0
                min_keep = max(cfg.min_target_hits, math.ceil(current_stats["target_hits"] * 0.60))
                for t, _n in addon_counts.most_common(refine_pool_cap):
                    if t in current:
                        continue
                    target_eval_count += 1
                    cand_match_all = current_match_all & trait_index.get(t, set())
                    stats = _fast_stats(rows, level, target, cand_match_all, remaining, target_idxs_all)
                    if stats["target_hits"] < min_keep:
                        continue
                    margin = stats["target_hits_all"] - stats["sibling_hits"]
                    cur_margin = current_stats["target_hits_all"] - current_stats["sibling_hits"]
                    precision_gain = stats["precision"] - current_stats["precision"]
                    margin_gain = margin - cur_margin
                    if precision_gain < 0.03 and margin_gain <= 0:
                        continue
                    score = (stats["target_hits"] * 80.0) + (stats["precision"] * 15.0) + max(0, margin) * 0.10
                    if score > add_score:
                        add_score = score
                        add_best = tuple(sorted(current + (t,)))
                        add_stats = stats
                        add_match_all = cand_match_all
                if add_best is None:
                    break
                current, current_stats, current_match_all = add_best, add_stats, set(add_match_all)

            covered_remaining = current_match_all & remaining
            new_target_hits = len(covered_remaining)
            if new_target_hits < cfg.min_target_hits:
                break
            ...
            profile_rows.append({
                ...
                "coverage_of_remaining": round(len(covered_remaining) / len(remaining), 4) if remaining else 0,
                "needs_separator": bool(current_stats["precision"] < cfg.min_precision),
                "eval_count_target_so_far": target_eval_count,
                "target_elapsed_seconds": round(time.time() - target_t0, 2),
                "production_status": "DISCOVERY_PROFILE_NOT_PRODUCTION_RULE",
            })
            remaining -= covered_remaining
        ...
        residual_rows.append({
            ...
            "timed_out_safety_stop": bool(timed_out),
        })

    if progress is not None:
        progress.progress(1.0)
    if status is not None:
        status.success(f"Finished {level} profile decomposition for {len(targets):,} targets in {round(time.time()-t0_all,2):,} seconds.")
    return pd.DataFrame(profile_rows), pd.DataFrame(residual_rows)



# -------------------------
# UI — v30 lifecycle-safe
# -------------------------
upload = st.file_uploader("Upload clean Pick-4 history CSV/TXT", type=["csv","txt","tsv"])

with st.sidebar:
    st.header("v30 settings")
    ...
    export_top_n = st.slider("CSV export top N rows", 25, 1000, 250, 25)
    st.divider()
    target_start = st.number_input("Chunk start target #", min_value=1, max_value=360, value=1, step=1)
    core_chunk_size = st.slider("Core chunk size", 5, 120, 10, 5)
    member_chunk_size = st.slider("Member chunk size", 5, 360, 20, 5)
    st.caption("v30: download reruns do not reload history. Start with 10 cores until download survives repeatedly.")
...
for k, v in {
    ...
    "v30_job_key": None,
    "v30_file_name": None,
    "v30_manifest_records": None,
    "v30_manifest_text": None,
}.items():
...
# Stable key must not require reading the file. This lets download reruns exit early.
job_key = "|".join(map(str, [
    APP_VERSION, getattr(upload, "name", "uploaded"), stage, max_profiles, max_depth, min_hits,
    min_precision, top_trait_pool, export_top_n, target_start, core_chunk_size, member_chunk_size
]))

st.subheader("Frozen download")
package_ready = st.session_state.get("v30_package") is not None and st.session_state.get("v30_job_key") == job_key
if package_ready:
    st.success(f"Frozen ZIP ready: {st.session_state.get('v30_file_name')}")
    st.download_button(
        "Download frozen v30 ZIP — no rebuild",
        st.session_state["v30_package"],
        file_name=st.session_state["v30_file_name"],
        mime="application/zip",
        key="download_frozen_v30_zip",
    )
    if st.session_state.get("v30_manifest_text"):
        st.text(st.session_state["v30_manifest_text"])
    st.caption("Download reruns stop here. History is not reloaded and mining is not rebuilt.")
    st.stop()
else:
    st.info("No frozen ZIP for these exact settings yet. Click Run once. Changing settings intentionally requires a new run.")

run_now = st.button("Run selected v30 stage and prepare frozen ZIP", type="primary")
if not run_now:
    st.stop()
...
try:
    ...
    manifest_rows = []
    frames = {}
    texts = {"README_v30.txt": f"{BUILD_MARKER}\n{DEPLOY_FILENAME_NOTE}\nStage run: {stage}\nDiscovery lab only. v30 freezes ZIP and exits before reparsing on download reruns.\n"}
    ...
    summary_rows = [
        ...
        {"metric":"parse_seconds", "value":parse_seconds},
        {"metric":"download_mode", "value":"early_exit_on_download_rerun_no_history_reload"},
    ...
    if stage.startswith("1"):
        status.info("Building base summaries...")
        ...
        frames["core_base_counts.csv"] = core_counts
        frames["member_base_counts.csv"] = member_counts
        frames["stream_core_base_counts.csv"] = stream_core.head(export_top_n)
        # keep prepared sample tiny only
        frames["prepared_aabc_rows_sample.csv"] = rows.drop(columns=["trait_list"]).head(min(export_top_n, 100))
        manifest_rows += [
            {"file":"core_base_counts.csv", "rows":len(core_counts)},
            {"file":"member_base_counts.csv", "rows":len(member_counts)},
            {"file":"stream_core_base_counts.csv", "rows":min(len(stream_core), export_top_n)},
            {"file":"prepared_aabc_rows_sample.csv", "rows":min(len(rows), min(export_top_n, 100))},
        ]
        status.success("Base summaries complete.")
        ...
        frames["core_decomposed_profiles.csv"] = prof.head(export_top_n).copy()
        frames["core_profile_residuals.csv"] = residual.copy()
        manifest_rows += [
            {"file":"core_decomposed_profiles.csv", "rows":len(frames["core_decomposed_profiles.csv"]), "full_rows_found":len(prof)},
            {"file":"core_profile_residuals.csv", "rows":len(residual)},
        ]
        summary_rows += [
            {"metric":"profiles_found", "value":len(prof)},
            {"metric":"residual_rows", "value":len(residual)},
        ]
        status.success(f"Stage 2 complete. Profiles found: {len(prof):,}. Residual targets: {len(residual):,}.")
        del prof, residual
        ...
        frames["member_decomposed_profiles.csv"] = prof.head(export_top_n).copy()
        frames["member_profile_residuals.csv"] = residual.head(export_top_n).copy()
        manifest_rows += [
            {"file":"member_decomposed_profiles.csv", "rows":len(frames["member_decomposed_profiles.csv"]), "full_rows_found":len(prof)},
            {"file":"member_profile_residuals.csv", "rows":len(frames["member_profile_residuals.csv"]), "full_rows_found":len(residual)},
        ]
        summary_rows += [
            {"metric":"profiles_found", "value":len(prof)},
            {"metric":"residual_rows", "value":len(residual)},
        ]
        status.success(f"Stage 3 complete. Profiles found: {len(prof):,}. Residual targets: {len(residual):,}.")
        del prof, residual

    elif stage.startswith("4"):
        with st.spinner("Building core profiles, scanning collisions, refining separators..."):
            prof, residual = build_profiles(rows, "core", cfg_core, target_start=int(target_start), target_chunk_size=int(core_chunk_size), progress=progress, status=status)
            ...
            sep = separator_refinement(rows, prof, "core", max_collisions=100, min_hits=max(3, min_hits//2))
        frames["core_decomposed_profiles.csv"] = prof.head(export_top_n).copy()
        frames["core_profile_residuals.csv"] = residual.copy()
        frames["core_profile_collisions.csv"] = collisions.head(export_top_n).copy()
        frames["core_collision_separator_refinements.csv"] = sep.head(export_top_n).copy()
        manifest_rows += [
            {"file":"core_decomposed_profiles.csv", "rows":len(frames["core_decomposed_profiles.csv"]), "full_rows_found":len(prof)},
            {"file":"core_profile_residuals.csv", "rows":len(residual)},
            {"file":"core_profile_collisions.csv", "rows":len(frames["core_profile_collisions.csv"]), "full_rows_found":len(collisions)},
            {"file":"core_collision_separator_refinements.csv", "rows":len(frames["core_collision_separator_refinements.csv"]), "full_rows_found":len(sep)},
        ]
        summary_rows += [
            {"metric":"profiles_found", "value":len(prof)},
            {"metric":"collisions_found", "value":len(collisions)},
            {"metric":"separator_refinements_found", "value":len(sep)},
        ]
        status.success(f"Stage 4 complete. Profiles: {len(prof):,}; collisions: {len(collisions):,}; refinements: {len(sep):,}.")
        del prof, residual, collisions, sep

    elif stage.startswith("5"):
        with st.spinner("Building member profiles, scanning collisions, refining separators..."):
            prof, residual = build_profiles(rows, "member", cfg_member, target_start=int(target_start), target_chunk_size=int(member_chunk_size), progress=progress, status=status)
            status.info("Scanning member profile collisions...")
            collisions = collision_scan(prof)
            status.info("Refining separators for member profile collisions...")
            sep = separator_refinement(rows, prof, "member", max_collisions=150, min_hits=max(3, min_hits//2))
        frames["member_decomposed_profiles.csv"] = prof.head(export_top_n).copy()
        frames["member_profile_residuals.csv"] = residual.head(export_top_n).copy()
        frames["member_profile_collisions.csv"] = collisions.head(export_top_n).copy()
        frames["member_collision_separator_refinements.csv"] = sep.head(export_top_n).copy()
        manifest_rows += [
            {"file":"member_decomposed_profiles.csv", "rows":len(frames["member_decomposed_profiles.csv"]), "full_rows_found":len(prof)},
            {"file":"member_profile_residuals.csv", "rows":len(frames["member_profile_residuals.csv"]), "full_rows_found":len(residual)},
            {"file":"member_profile_collisions.csv", "rows":len(frames["member_profile_collisions.csv"]), "full_rows_found":len(collisions)},
            {"file":"member_collision_separator_refinements.csv", "rows":len(frames["member_collision_separator_refinements.csv"]), "full_rows_found":len(sep)},
        ]
        summary_rows += [
            {"metric":"profiles_found", "value":len(prof)},
            {"metric":"collisions_found", "value":len(collisions)},
        ...
        status.success(f"Stage 5 complete. Profiles: {len(prof):,}; collisions: {len(collisions):,}; refinements: {len(sep):,}.")
        del prof, residual, collisions, sep

    mining_seconds = round(time.time() - t_mine, 3)
    summary_rows.append({"metric":"mining_seconds", "value":mining_seconds})

    # Build manifest and package while keeping only small outputs.
    manifest = pd.DataFrame(manifest_rows) if manifest_rows else pd.DataFrame([{"file":"summary.csv", "rows":0}])
    summary = pd.DataFrame(summary_rows)
    frames["summary.csv"] = summary
    frames["manifest.csv"] = manifest

    t_zip = time.time()
    st.info("Finalizing ZIP package. No previews will be rendered.")
    package = zip_frames(frames, texts)
    zip_seconds = round(time.time() - t_zip, 3)
    out_name = f"core_affinity_lab_v30_stage_{stage.split()[0]}_chunk_{int(target_start)}.zip"
    zip_size_mb = round(len(package) / (1024*1024), 4)

    # Convert manifest to plain text before deleting dataframes.
    manifest_records = manifest.to_dict("records")
    manifest_text_lines = [
        f"ZIP: {out_name}",
        f"ZIP size MB: {zip_size_mb}",
        f"Parse seconds: {parse_seconds}",
        f"Mining seconds: {mining_seconds}",
        f"ZIP seconds: {zip_seconds}",
        "Files:",
    ]
    for r in manifest_records:
        detail = ", ".join(f"{k}={v}" for k, v in r.items())
        manifest_text_lines.append(f"- {detail}")
    manifest_text = "\n".join(manifest_text_lines)

    # Clear all large objects before exposing download.
    cleanup_t = time.time()
    try:
        del frames, texts, manifest, summary, rows
    except Exception:
        pass
    gc.collect()
    cleanup_seconds = round(time.time() - cleanup_t, 3)
    manifest_text += f"\nCleanup seconds: {cleanup_seconds}\nTotal seconds: {round(time.time() - t_total, 3)}"

    # Store only compact final artifacts in session_state.
    st.session_state["v30_package"] = package
    st.session_state["v30_job_key"] = job_key
    st.session_state["v30_file_name"] = out_name
    st.session_state["v30_manifest_records"] = manifest_records
    st.session_state["v30_manifest_text"] = manifest_text

    st.success(f"Frozen ZIP prepared: {out_name} ({zip_size_mb} MB).")
    ...
    st.download_button(
        ...
        file_name=st.session_state["v30_file_name"],
        mime="application/zip",
        key="download_frozen_v30_zip_after_run",
    )

except Exception as e:
    st.error("v30 stage failed. Full traceback below.")
    st.exception(e)
