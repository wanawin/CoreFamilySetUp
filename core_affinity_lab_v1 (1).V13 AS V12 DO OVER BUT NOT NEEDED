...
st.set_page_config(page_title="Core Affinity Lab v13", layout="wide")
st.title("Core Affinity Lab v13 — Signal Validation + Core/Member Separators")
...
if st.button("Run selected v13 stage", type="primary", use_container_width=True):
    try:
        ...
        zip_bytes = make_zip(frames, {"README_v13.txt": "Core Affinity Lab v13 signal-validation output. Run one stage at a time; combine exported CSVs later. Lab only; no playlist logic."})
        freeze_bytes("v13_last_zip", zip_bytes)
        st.success("Stage complete. Download ZIP below.")
        st.download_button("Download v13 selected-stage outputs ZIP", data=zip_bytes, file_name="core_affinity_lab_v13_selected_stage_outputs.zip", mime="application/zip", use_container_width=True)

    except Exception as e:
        st.error("v13 caught the error instead of blank-crashing.")
        st.exception(e)
        st.stop()

if "v13_last_zip" in st.session_state:
    st.download_button("Download previous v13 ZIP again", data=st.session_state["v13_last_zip"], file_name="core_affinity_lab_v13_selected_stage_outputs.zip", mime="application/zip", use_container_width=True)
