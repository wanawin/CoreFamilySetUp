#!/usr/bin/env python3
"""
Core Affinity Lab v34 — Beam Search Collision Separator Miner
...
APP_VERSION = "v34"
BUILD_MARKER = "BUILD: core_affinity_lab_v34_BEAM_SEARCH_COLLISION_MINER__2026-06-18"
DEPLOY_FILENAME_NOTE = "For Streamlit Cloud deployment, this file may be renamed to: core_affinity_lab_v1 (1).py"

st.set_page_config(page_title="Core Affinity Lab v34", layout="wide")
st.title("Core Affinity Lab v34 — Beam Search Collision Separator Miner")
st.caption(BUILD_MARKER)
st.info(
    "Upload clean history plus all v30 Stage-2 core-profile ZIP chunks. "
    "v34 merges v30 coverage buckets globally, deduplicates overlaps, then mines BEAM-SEARCH separators: target rows vs sibling rows inside each broad bucket. "
...
def mine_profile_separators(
    ...
    """v34 beam-search collision separator miner.

    v34 compared target rows vs sibling rows, but the separator search was still too shallow.
    v34 uses a bounded beam search similar to the older deep miners:

        base coverage profile
        -> level-1 discriminators
        -> keep top beam
        -> expand each stack to level 2/3/4
        -> rank by precision gain + target-vs-sibling lift + odds ratio + retained coverage

    Discovery only. Not production play logic.
    """
    ...
    beam_width = max(3, int(top_per_profile))
    max_depth = max(1, min(4, int(refine_depth)))
    level1_pool = int(candidate_pool)
    addon_pool = max(20, min(int(candidate_pool), 100))
    ...
    def is_redundant_with_stack(trait: str, stack: Tuple[str, ...]) -> bool:
        ...
        if m and f"no{m.group(1)}=1" in base:
            return True
        # mirror_ge flags can be redundant with exact count in same stack only as a soft duplicate.
        return False
    ...
    def score_matched(target_set: Set[int], sibling_set: Set[int], matched: Set[int], total_target_hits: int, base_precision: float, profile_target_total: int) -> dict:
        ...
        # Beam score: not production rank. It keeps promising branches alive.
        # Precision/lift matter, but coverage prevents tiny perfect traps from dominating.
        score = (
            max(0.0, precision_gain) * 900.0
            + min(lift, 80.0) * 10.0
            + min(odds_ratio, 150.0) * 3.0
            + profile_coverage * 180.0
            + global_coverage * 40.0
            + target_hits * 0.55
            + max(0, hit_margin) * 0.05
        )
        return {
            "sample": sample,
    ...
    def row_from_candidate(pr, target, base_stack, sep_stack, bucket, bucket_target, bucket_sibling, stats, base_precision, base_target_coverage):
        return {
            ...
            "candidate_rank_basis": "beam_search: precision_gain + lift + odds_ratio + retained_coverage",
    ...
    for n, (_, pr) in enumerate(selected.iterrows(), start=1):
        ...
        if status is not None:
            status.info(f"Beam-search separators: profile {n}/{total} — core {target} profile {pr.get('profile_num','')}")
        ...
        audit = {
            ...
            "max_depth": max_depth,
            "note": "v34 uses beam-search stack expansion against target rows vs sibling rows inside this bucket.",
        }
        if len(bucket_target) < min_target_hits or not bucket:
            audit_rows.append(audit)
            continue

        # Level 1 from target rows, scored against siblings.
        target_counts = count_traits(rows, bucket_target)
        beam = []  # list of (score, sep_stack_tuple, matched_set, stats)
        seen = set()
        examined_l1 = 0
        for trait, _cnt in target_counts.most_common(level1_pool):
            ...
            if not stats or stats["target_hits"] < min_target_hits:
                continue
            # Keep weak-but-promising candidates for expansion if they have lift or coverage.
            if stats["target_vs_sibling_lift"] <= 1.01 and stats["precision_gain"] <= 0:
                continue
            sep_stack = (trait,)
            seen.add(sep_stack)
            beam.append((stats["score"], sep_stack, matched, stats))
        ...
        # Expand beam level by level. Add-on traits are counted from the CURRENT target hits after stack.
        current_beam = beam
        total_examined = examined_l1
        for depth in range(2, max_depth + 1):
            next_candidates = []
            for _score, sep_stack, matched_prev, stats_prev in current_beam:
                target_after = matched_prev & bucket_target
                sibling_after = matched_prev & bucket_sibling
                ...
                for trait, _cnt in addon_counts.most_common(addon_pool):
                    if is_redundant_with_stack(trait, base_stack + sep_stack):
                        continue
                    local_examined += 1
                    total_examined += 1
                    ...
                    if not stats or stats["target_hits"] < min_target_hits:
                        continue
                    # Do not keep stacks that only shrink coverage without improving discrimination.
                    if stats["precision"] + 1e-9 < stats_prev["precision"] and stats["target_vs_sibling_lift"] <= stats_prev["target_vs_sibling_lift"]:
                        continue
                    # Must either improve precision, improve lift, or materially reduce siblings while preserving target hits.
                    if (stats["precision_gain"] <= 0 and stats["target_vs_sibling_lift"] <= 1.05 and stats["hit_margin"] <= stats_prev["hit_margin"]):
                        continue
                    next_candidates.append((stats["score"], new_sep_stack, matched, stats))
                    sep_rows.append(row_from_candidate(pr, target, base_stack, new_sep_stack, bucket, bucket_target, bucket_sibling, stats, base_precision, base_target_coverage))
                # bounded search guard
                if local_examined >= addon_pool:
                    pass
            if not next_candidates:
                break
            # Keep distinct candidates; prefer coverage diversity, but avoid huge overlap not implemented here for speed.
            current_beam = sorted(next_candidates, key=lambda x: x[0], reverse=True)[:beam_width]

        audit["level1_examined"] = examined_l1
        audit["total_candidates_examined"] = total_examined
        audit["separator_rows_generated_for_profile"] = sum(1 for r in sep_rows if r.get("profile_row_num") == int(pr.get("profile_row_num", n)))
        audit_rows.append(audit)

    if progress is not None:
        progress.progress(1.0)
    if status is not None:
        status.success(f"Finished v34 beam-search separator mining for {len(selected)} profiles in {time.time()-t0:.2f} seconds.")
...
with st.sidebar:
    ...
    candidate_pool = st.slider("Separator candidate trait pool", 25, 500, 220, 5)
    beam_width = st.slider("Beam width per profile", 5, 75, 25, 5)
    refine_depth = st.slider("Beam-search separator max depth", 1, 4, 3)
...
if "v34_package" not in st.session_state:
    st.session_state["v34_package"] = None
    st.session_state["v34_manifest"] = None
    st.session_state["v34_filename"] = None
...
if run:
    ...
    try:
        ...
        texts["README_v34.txt"] = (
            ...
            "v34 consumes v30 coverage profiles and mines discriminative separator traits using target-vs-sibling frequency, lift, odds ratio, and precision gain.\n"
        )
        package = zip_frames(frames, texts)
        filename = f"core_affinity_lab_v34_stage_{stage.split()[0]}_profiles_{int(profile_start)}.zip"
        st.session_state["v34_package"] = package
        st.session_state["v34_manifest"] = manifest
        st.session_state["v34_filename"] = filename
...
if st.session_state.get("v34_manifest") is not None:
    st.subheader("Frozen package manifest")
    st.table(safe_display_df(st.session_state["v34_manifest"]))

if st.session_state.get("v34_package") is not None:
    st.download_button(
        "Download frozen v34 stage outputs ZIP",
        st.session_state["v34_package"],
        file_name=st.session_state.get("v34_filename", "core_affinity_lab_v34_outputs.zip"),
